...
function renderShopItems(animate = true) {
    ...
    backgroundsContainer.innerHTML = backgrounds.map(item => {
        ...
        if (isRestricted) {
            ...
            subtitle = `<span style="color: #ef4444; font-weight: 700; font-size: 0.7em;">DEFAULT THEME ONLY</span>`;
            rightIcon = ``;
        } else if (isSelected) {
            itemClasses += ' selected';
            subtitle = `<span>Débloqué</span>`;
            ...
            itemClasses += ' buyable';
            subtitle = `<span style="color: var(--active-color); font-weight: 700;">${item.cost} ${COIN_SVG}</span>`;
            rightIcon = `<i class="fas fa-chevron-right" style="color: var(--icon-color);"></i>`;
        }
        ...
        return `
            <div class="${itemClasses}" data-item-id="${item.id}" data-theme="${isPurchased && !isRestricted ? item.id : ''}">
                ...
                ${isSelected || isPurchased ? rightIcon : ''}
            </div>`;
    }).join('');
    ...
    themesContainer.innerHTML = visibleThemes.map(item => {
        ...
        if (item.id === 'theme-ds' && window.innerWidth > 952) {
            subtitle = `<span style="color: #ef4444; font-weight: 700; font-size: 0.7em;">MOBILE ONLY</span>`;
            rightIcon = ``;
        } else if (isSelected) {
            subtitle = `<span>${getTranslation('themeUnlocked')}</span>`;
            rightIcon = `<i class="fas fa-check" style="color: var(--active-color);"></i>`;
        } else if (isUnlocked) {
            subtitle = `<span>${getTranslation('themeUnlocked')}</span>`;
            rightIcon = `<div style="width: 20px; height: 20px; border: 2px solid var(--text-muted); border-radius: 50%;"></div>`;
            ...
            // Locked
            subtitle = `<span style="font-size: 0.75em;">${missionDesc} (${currentProgress}/${goal})</span>`;
        ...
        return `
            <div class="${itemClasses}" data-item-id="${item.id}" data-theme="${isUnlocked ? item.id : ''}" data-mission-id="${item.missionId || ''}">
                ...
                ${isSelected || isUnlocked ? rightIcon : ''}
            </div>
        `;
    }).join('');
                ...
                html += tracks.map(item => `
                    <div class="shop-list-item locked buyable ${animate ? 'animated' : ''}" data-item-id="${item.id}">
                        ...
                        <div class="shop-list-content">
                            ...
                            <p class="shop-list-subtitle" style="color: var(--active-color); font-weight: 700;">${COIN_COST_UNLOCK} ${COIN_SVG}</p>
                        </div>
                        <i class="fas fa-chevron-right" style="color: var(--icon-color); font-size: 0.9em;"></i>
                    </div>`).join('');
    ...
    container.innerHTML = `
        ...
        <div id="panel-1" class="accordion-panel active" onclick="toggleAccordion('panel-1')">
            ...
            <div class="panel-content">
                ...
                <p class="hero-subtitle">Dernier single disponible sur toutes les plateformes</p>
        ...
        <div id="panel-2" class="accordion-panel" onclick="toggleAccordion('panel-2')">
            ...
            <div class="panel-content">
                ...
                <p class="hero-subtitle">Bientôt disponible - Restez à l'écoute !</p>
