...
window._ytReadyCallback = initYouTubePlayers;
...
function playAudio(audioElement, force = false) {
    ...
    // NOUVEAU: Verrou pour empêcher les sons de se chevaucher (sauf si forcé)
    if (sfxLock && !force) return;
        ...
        return `
            <div class="${itemClasses}" data-item-id="${item.id}" data-theme="${isPurchased && !isRestricted ? item.id : ''}">
                <div class="shop-list-img">
                    <img src="${item.image}" alt="${itemName}" width="60" height="60" loading="lazy">
...
function updateThemeColorMeta(forceIntro = false) {
    ...
    if (isIntroVisible) {
        ...
        if (document.body.classList.contains('theme-playstation')) {
            ...
            navColor = '#002d5c';   // Bottom Bar PS3
            ...
            themeColor = '#f4f3f3'; // Top Bar Spotimon (couleur du header mobile)
            navColor = '#111111';   // Bottom Bar Spotimon
        } else if (document.body.classList.contains('theme-16bit')) {
            themeColor = '#f6fcfe'; // Frutiger Aero (couleur du header mobile)
            navColor = '#ebf8ff';   // Frutiger Aero (bas)
        } else if (document.body.classList.contains('theme-default-2')) {
            themeColor = '#ffffff'; // Default 2 : header blanc solide
            navColor = '#ffffff';
        } else {
            // Thème Default : header blanc solide
            themeColor = '#ffffff';
            navColor = '#ffffff';
        }
    }
...
function openOverlay(overlay, sound, keepMusicPlaying = false) {
    ...
    if (overlay.id === 'wifi-overlay') {
        // Lazy load : on injecte le src seulement au premier appel
        if (sounds.connecting && !sounds.connecting.getAttribute('src')) {
            sounds.connecting.src = 'assets/sounds/connecting.ogg';
            sounds.connecting.load();
        }
...
function updateProgressBar() {
    ...
    if (duration > 0) { // If duration is valid
        ...
        // NOUVEAU: Sécurisation des mises à jour de l'UI pour éviter les erreurs si un élément est manquant
        const desktopProgressBar = document.getElementById('progress-fill');
        if (desktopProgressBar) {
            desktopProgressBar.style.width = `${listenProgress * 100}%`;
        }
        const desktopCurrentTime = document.getElementById('current-time-display');
        if (desktopCurrentTime) {
            desktopCurrentTime.textContent = formatTime(currentTime);
        }
        const desktopDuration = document.getElementById('duration-display');
        if (desktopDuration) {
            desktopDuration.textContent = formatTime(duration);
        }
...
function showSection(sectionId, updateHistory = true, previousContext = null) {
    ...
    if (updateHistory) {
        const currentContextStr = JSON.stringify(currentViewContext);
        const historyContextStr = history.state ? JSON.stringify(history.state.context) : null;
        const isNewSection = !history.state || history.state.section !== sectionId;
        const isNewContext = currentContextStr !== historyContextStr;

        if (isNewSection || isNewContext) {
            // Mettre à jour l'entrée actuelle de l'historique avec son vrai contexte avant de la quitter
            const contextToSave = previousContext || (history.state ? history.state.context : { ...currentViewContext });
            history.replaceState({ ...history.state, context: contextToSave }, '', window.location.hash);

            // Pousser la nouvelle entrée avec le nouveau contexte
            const stateToPush = { section: sectionId, fromApp: true, context: { ...currentViewContext } };
            history.pushState(stateToPush, '', '#' + sectionId);
        } else {
            // Même section et même contexte, on s'assure juste que l'état actuel est synchronisé
...
function updateVisibleCards(customTitle = null, animate = true) {
    ...
    currentViewContext = { type: 'search', data: query }; // CORRECTION: Store query for restoration
    showSection('search-results-section', true);
...
function applyTheme(themeName) {
    ...
    if (isBgTheme) {
        ...
        // 2. Ajouter le nouveau thème (sauf si c'est le défaut original 'default')
...
// NOUVEAU: Réinitialise les filtres par tag.
function resetTagFilters() {
    // Décoche toutes les cases de filtre de tags
    document.querySelectorAll('#tags-filter-list input:checked').forEach(cb => {
        cb.checked = false;
    });

    // Met à jour les badges de compteur de filtre
    const updateBadge = (id) => {
        const badge = document.getElementById(id);
        if (badge) {
            badge.textContent = '';
    ...
    // Cache le point sur l'icône de recherche mobile
    const filterDotOnSearch = document.querySelector('.filter-dot-on-search');
    if (filterDotOnSearch) {
        filterDotOnSearch.classList.add('hidden');
    }
}
...
function renderSidebarNav(activeProfile, currentSectionId = null) {
    ...
    const svgBase = '<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="sidebar-icon">';
    
    // CORRECTION: Utilisation des SVG directs pour éviter les bugs de chargement Lucide.
    const iconMap = {
        'albums': svgBase + '<rect width="18" height="18" x="3" y="3" rx="2" /><circle cx="12" cy="12" r="5" /><path d="M12 12h.01" /></svg>',
        'albumsBeats': svgBase + '<rect width="18" height="18" x="3" y="3" rx="2" /><circle cx="12" cy="12" r="5" /><path d="M12 12h.01" /></svg>',
        'videos': svgBase + '<rect width="20" height="15" x="2" y="7" rx="2" /><polyline points="17 2 12 7 7 2" /></svg>',
        'about': svgBase + '<circle cx="12" cy="12" r="10"/><path d="M12 16v-4"/><path d="M12 8h.01"/></svg>',
        'library': svgBase + '<path d="m16 6 4 14" /><path d="M12 6v14" /><path d="M8 8v12" /><path d="M4 4v16" /></svg>',
        'shop': svgBase + '<path d="M16 10a4 4 0 0 1-8 0" /><path d="M3.103 6.034h17.794" /><path d="M3.4 5.467a2 2 0 0 0-.4 1.2V20a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2V6.667a2 2 0 0 0-.4-1.2l-2-2.667A2 2 0 0 0 17 2H7a2 2 0 0 0-1.6.8z" /></svg>',
        'home': svgBase + '<path d="M15 21v-8a1 1 0 0 0-1-1h-4a1 1 0 0 0-1 1v8" /><path d="M3 10a2 2 0 0 1 .709-1.528l7-6a2 2 0 0 1 2.582 0l7 6A2 2 0 0 1 21 10v9a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z" /></svg>',
        'default': svgBase + '<circle cx="12" cy="12" r="10"/><path d="M9.09 9a3 3 0 0 1 5.83 1c0 2-3 3-3 3"/><path d="M12 17h.01"/></svg>'
    };
    ...
    const profileLinks = otherProfileItems.map(item => { // Map other profile items to links
        ...
        const iconSvg = iconMap[item.langKey] || iconMap.default;
        return `
...
function initializeApp() {
    ...
    if ('serviceWorker' in navigator) {
        ...
        // NOUVEAU: Recharger la page quand le nouveau SW prend le contrôle (mais pas à la première visite)
        let refreshing;
        let isFirstRegistration = !navigator.serviceWorker.controller;
        navigator.serviceWorker.addEventListener('controllerchange', () => {
            if (refreshing || isFirstRegistration) return;
...
function setupEventListeners() {
    ...
    document.getElementById('mini-player-close-btn').addEventListener('click', (e) => {
        ...
        const wasOnDetailsPage = !document.getElementById('music-title-details-section').classList.contains('hidden');
        if (activePlayer && typeof activePlayer.stopVideo === 'function') activePlayer.stopVideo();
        currentPlayingItem = null;
        resetMiniPlayerUI();
        if (wasVideo || wasOnDetailsPage) {
            handleMenuNavigation('home-dashboard-section');
    }
});
...
function handleMenuNavigation(dataLink, updateHistory = true, context = null) {
    // NOUVEAU: Réinitialiser les filtres par tag lors d'une navigation principale.
    // On ne le fait que si `updateHistory` est vrai, pour ne pas réinitialiser
    // lors d'un simple rafraîchissement de vue (ex: changement grille/liste).
    if (updateHistory) {
        resetTagFilters();
    }
    ...
    if (dataLink === 'home-dashboard-section') {
        ...
        if (effectiveContext && effectiveContext.type === 'titles' && effectiveContext.data) {
            ...
            const album = siteData.contentData[activeProfile].albums[albumId];
            if (album) {
                document.getElementById('titles-section-title').textContent = album.title;
            }
            renderCards('titles-cards', titlesForAlbum, 'title');
