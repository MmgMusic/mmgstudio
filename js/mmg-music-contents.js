...
function updateNotificationDot() {
    ...
    // MODIFICATION: Le point rouge apparaît pour les messages non lus OU si on peut débloquer quelque chose
...
function openOverlay(overlay, sound, keepMusicPlaying = false) {
    ...
    // NOUVEAU: On ne marque plus comme lu automatiquement à l'ouverture pour éviter de masquer les notifications importantes
    if (overlay.id === 'notifications-overlay') {
        renderUpdateLog(); // S'assure que le contenu est à jour
    }
...
function renderCards(containerId, cardsData, cardType = 'generic', animate = true) {
    ...
    if (isListView) {
        ...
        const listSelectors = `#${containerId} .list-view-title > span, #${containerId} .list-view-artist-col > span`;
        ...
        const gridSelector = `#${containerId} .card__title > span`;
...
function renderDailyBonusProgress() {
    ...
    // NOUVEAU: Écouteur pour le bouton de test des notifications
    const testNotifBtn = document.getElementById('test-notification-btn');
    if (testNotifBtn) {
        testNotifBtn.addEventListener('click', async () => {
            playAudio(sounds.select);
            if (!('Notification' in window)) {
                showDialog("Votre navigateur ne supporte pas les notifications.");
                return;
            }
            ...
            if (permission === 'granted') {
                if ('serviceWorker' in navigator) {
                    const registration = await navigator.serviceWorker.ready;
                    registration.showNotification('Mmg Music', {
                    ...
                    new Notification('Mmg Music', {
                        body: 'Test de notification réussi !',
                        icon: './assets/icons/icon-192.webp'
                    });
                }
            } else {
                showDialog("Permissions de notification refusées.");
            }
        });
    }

    // NOUVEAU: Initialiser l'état du switch notification
    const notificationSwitch = document.getElementById('notifications-switch');
    if (notificationSwitch) {
        notificationSwitch.checked = Notification.permission === 'granted';
        notificationSwitch.addEventListener('change', async () => {
            if (notificationSwitch.checked) {
                const permission = await Notification.requestPermission();
                notificationSwitch.checked = (permission === 'granted');
            }
        });
    }
}
...
const playNextTrack = (direction = 1, forcePlay = false, isSwipe = false, promptUser = false) => {
    ...
    if (isShuffleMode && forcePlay) {
        ...
        nextItemId = currentQueue[randomIndex];
    }
...
function setupEventListeners() {
    ...
    mainWrapper.addEventListener('click', (e) => { // Add click listener to main wrapper
        ...
        const link = e.target.closest('a.card-link-wrapper, .sidebar-nav-link, .dashboard-card-button');
        if (!link) return; // If no link, return
        e.preventDefault();
        const { youtubeId, link: dataLink, albumId, unlockAlbum, itemId: linkItemId } = link.dataset || {};
        const cardElement = link.closest('.card'); // CORRECTION: Inclut .carousel-item pour trouver l'ID
...
/**
 * NOUVEAU: Marque une notification spécifique comme lue.
 */
function markNotificationAsRead(id) {
    if (!id) return;
    readUpdateIds.add(id);
    safeStorage.setItem('mmg-readUpdateIds', JSON.stringify([...readUpdateIds]));
    updateNotificationDot();
    renderUpdateLog();
}
...
function renderUpdateLog() {
    ...
    // --- Logique pour les messages réels (Log et Dev) ---
    const allMessages = [
        ...(siteData.updateLog || []).map(m => ({ ...m, type: 'log' })),
        ...(siteData.devMessages || []).map(m => ({ ...m, type: 'dev' }))
    ].sort((a, b) => new Date(b.date || 0) - new Date(a.date || 0));

    const messagesHtml = allMessages.map(msg => {
        const isRead = readUpdateIds.has(msg.id);
        const icon = msg.type === 'dev' ? 'fa-code' : 'fa-info-circle';
        return `
            <div class="notification-item ${isRead ? 'read' : 'unread'}" data-id="${msg.id}" onclick="markNotificationAsRead('${msg.id}')">
                <div class="notification-icon">
                    <i class="fas ${icon}"></i>
                </div>
                <div class="notification-details">
                    <div style="display: flex; justify-content: space-between; align-items: baseline;">
                        <h4>${msg.type === 'dev' ? 'Message Développeur' : 'Mise à jour'}</h4>
                        <span style="font-size: 0.7em; opacity: 0.6;">${msg.date || ''}</span>
                    </div>
                    <p style="font-size: 0.8em; opacity: 0.9;">${msg.content}</p>
                </div>
    ...
    container.innerHTML = dynamicNotificationsHtml + messagesHtml;
}
