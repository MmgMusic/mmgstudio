...
let sidebarFloatingState = { isFloating: false, x: 50, y: 50, width: 260, height: window.innerHeight * 0.7 };
let playerFloatingState = { isFloating: false, x: window.innerWidth - 750, y: window.innerHeight - 130 };
...
function getActiveProfile() {
    // Vérifie d'abord le switch mobile qui est toujours présent dans le DOM
    const activeMobileBtn = document.querySelector('#mobile-profile-switch .profile-switch-btn.active');
    if (activeMobileBtn) return activeMobileBtn.dataset.project;

    // Fallback sur le bloc de marque (Studio)
    const branding = document.getElementById('mmg-branding');
    if (branding && branding.classList.contains('is-beats')) return 'mmg-beats';
...
/**
 * NOUVEAU: Rend un élément déplaçable.
 * @param {HTMLElement} elmnt - L'élément à déplacer.
 * @param {HTMLElement} handle - L'élément qui sert de poignée de déplacement.
 * @param {string} storageKey - La clé localStorage pour sauvegarder la position.
 */
function makeDraggable(elmnt, handle, storageKey) {
    ...
    function closeDragElement() {
        ...
        const state = JSON.parse(safeStorage.getItem(storageKey) || '{}');
        state.x = elmnt.offsetLeft;
        state.y = elmnt.offsetTop;
...
// NOUVEAU: Charger l'état flottant
const storedSidebarState = safeStorage.getItem('mmg-sidebar-floating-state');
if (storedSidebarState) sidebarFloatingState = JSON.parse(storedSidebarState);
const storedPlayerState = safeStorage.getItem('mmg-player-floating-state');
if (storedPlayerState) playerFloatingState = JSON.parse(storedPlayerState);
...
function updateNotificationDot() {
    ...
    // MODIFICATION: Le point rouge n'apparaît que pour les vrais messages
    const shouldShowDot = hasUnreadMessages;

    // On cache ou montre les points sur TOUS les éléments concernés
    dots.forEach(dot => {
        if (shouldShowDot) {
    ...
    if (markAsReadBtn) {
        // Le bouton est actif s'il y a des messages non lus
...
function applyLanguage(lang) {
    ...
    // NOUVEAU: Met à jour le style du sélecteur de langue moderne
    document.querySelectorAll('.lang-option').forEach(opt => {
        opt.classList.toggle('active', opt.classList.contains(lang));
    });

    // NOUVEAU: Force le re-rendu des notifications pour la langue
    renderUpdateLog();
...
function renderShopTabs(activeTabId) {
    ...
    container.querySelectorAll('.playlist-tab-btn').forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            // NOUVEAU: Maintient le contexte
            if (currentViewContext && currentViewContext.type === 'shop') {
                currentViewContext.data = tabId;
            }
        });
    });
}
...
window.showShopTab = function (tabId) {
    ...
    // NOUVEAU: Met à jour l'état actif des onglets (pour la navigation depuis les notifications)
    const tabsContainer = document.getElementById('shop-tabs-container');
    if (tabsContainer) {
        tabsContainer.querySelectorAll('.playlist-tab-btn').forEach(btn => {
        ...
        updateSlidingIndicator(tabsContainer);
    }
}
...
function renderShopItems(animate = true) {
    ...
    backgroundsContainer.innerHTML = backgrounds.map(item => {
        ...
        let subtitle = '';
        let rightIcon = '';
        let itemClasses = 'shop-list-item';
        if (animate) itemClasses += ' animated';

        const artistName = item.id.includes('beats') || item.profile === 'mmg-beats' ? 'Mmg Beats' : 'Mmg Music';

        if (isRestricted) {
            itemClasses += ' locked';
            subtitle = `<span style="color: #ef4444; font-weight: 700; font-size: 0.75em;">DEFAULT THEME ONLY</span>`;
            rightIcon = ``;
        } else if (isSelected) {
            itemClasses += ' selected';
            subtitle = `<span class="shop-item-status-text" style="color: var(--active-color); font-weight:600;">${getTranslation('themeUnlocked')}</span>`;
            rightIcon = `<i class="fas fa-check" style="color: var(--active-color);"></i>`;
        } else if (isPurchased) {
            subtitle = `<span class="shop-item-status-text" style="color: var(--text-muted);">${getTranslation('themeUnlocked')}</span>`;
            rightIcon = `<div style="width: 20px; height: 20px; border: 2px solid var(--text-muted); border-radius: 50%;"></div>`;
        } else {
            itemClasses += ' buyable';
            subtitle = `<span>${getTranslation('buyToUnlock')}</span>`;
            rightIcon = `<span class="shop-item-price-badge" style="display:flex; align-items:center; gap:3px; font-weight:700; font-size:0.85em; color: var(--active-color); white-space:nowrap; flex-shrink:0;">${item.cost} ${COIN_SVG}</span>`;
        }
        ...
        return `
            <div class="${itemClasses}" data-item-id="${item.id}" data-theme="${isPurchased && !isRestricted ? item.id : ''}">
                <div class="shop-list-img">
                    <img src="${item.image}" alt="${itemName}">
                    ${!isPurchased && !isRestricted ? `
                    <div style="position:absolute; inset:0; background:rgba(0,0,0,0.4); display:flex; align-items:center; justify-content:center; color:white;">
                        <i class="fas fa-lock" style="font-size: 0.9em;"></i>
                    </div>` : ''}
                </div>
                <div class="shop-list-content">
                    <p class="shop-list-title">${itemName}</p>
                    <p class="shop-list-subtitle">${subtitle}</p>
                </div>
    ...
    themesContainer.innerHTML = visibleThemes.map(item => {
        ...
        if (!isUnlocked) itemClasses += " locked";
        if (animate) itemClasses += " animated";

        // NOUVEAU: Restriction Mobile Only pour le thème DS
        if (item.id === 'theme-ds' && window.innerWidth > 952) {
            itemClasses += ' locked mobile-only';
        }
        ...
        if (item.id === 'theme-ds' && window.innerWidth > 952) {
            subtitle = `<span style="color: #ef4444; font-weight: 700; font-size: 0.75em;">MOBILE ONLY</span>`;
            rightIcon = ``;
        } else if (isSelected) {
            subtitle = `<span class="shop-item-status-text" style="color: var(--active-color); font-weight:600;">${getTranslation('themeUnlocked')}</span>`;
            rightIcon = `<i class="fas fa-check" style="color: var(--active-color);"></i>`;
            ...
            // Locked
            subtitle = `<span>${missionDesc} (${currentProgress}/${goal})</span>`;
            rightIcon = `<i class="fas fa-lock" style="color: var(--text-muted);"></i>`;
        }

        return `
            <div class="${itemClasses}" data-item-id="${item.id}" data-theme="${isUnlocked ? item.id : ''}" data-mission-id="${item.missionId || ''}">
                <div class="shop-list-img">
                    <img src="${item.image || 'assets/shopthemepreviews/Preview-default.webp'}" alt="${itemName}">
                    ${!isUnlocked ? `
                    <div style="position:absolute; inset:0; background:rgba(0,0,0,0.4); display:flex; align-items:center; justify-content:center; color:white;">
                        <i class="fas fa-lock" style="font-size: 0.9em;"></i>
                    </div>` : ''}
    ...
    if (tracksContainer) {
        ...
        if (lockedTracks.length > 0) {
            ...
            let html = '';
            for (const [albumId, tracks] of Object.entries(tracksByAlbum)) {
                // Trouver le titre de l'album et le nom de l'artiste
                let albumTitle = 'Album';
                let artistName = '';

                // On récupère le profil depuis le premier titre du groupe
                const profileId = tracks[0].profile;
                artistName = profileId === 'mmg-beats' ? 'Mmg Beats' : 'Mmg Music';

                for (const profile of Object.values(siteData.contentData)) {
                    if (profile.albums && profile.albums[albumId]) {
                        albumTitle = profile.albums[albumId].title;
                        break;
                    }
                }

                // Ajouter l'en-tête de section avec le nom de l'artiste
                html += `<div class="section-header-simple" style="margin-top: 20px; margin-bottom: 10px;">
                            <h3 class="section-title" style="margin: 0; display: flex; align-items: center; gap: 8px;">
                                ${albumTitle} 
                                <span style="opacity: 0.5;">${artistName}</span>
                            </h3>
                         </div>`;

                // Ouvrir le conteneur de liste pour cet album
                html += `<div class="shop-list-container">`;

                // Ajouter les titres
                html += tracks.map(item => {
                    const artistName = item.profile === 'mmg-beats' ? 'Mmg Beats' : 'Mmg Music';
                    return `
                    <div class="shop-list-item locked buyable ${animate ? 'animated' : ''}" data-item-id="${item.id}">
                        <div class="shop-list-img">
                            <img src="${getCorrectImagePath(item, 'thumb')}" alt="${item.title}">
                            <div style="position:absolute; inset:0; background:rgba(0,0,0,0.4); display:flex; align-items:center; justify-content:center; color:white;">
                                <i class="fas fa-lock" style="font-size: 0.9em;"></i>
                        ...
                        <div class="shop-list-content">
                            <p class="shop-list-title">${item.title}</p>
                            <p class="shop-list-subtitle">${getTranslation('buyToUnlock')}</p>
                        </div>
                        <div class="shop-list-actions">
                            <span class="shop-item-price-badge" style="display:flex; align-items:center; gap:3px; font-weight:700; font-size:0.85em; color: var(--active-color); white-space:nowrap; flex-shrink:0;">${COIN_COST_UNLOCK} ${COIN_SVG}</span>
                        </div>
                    </div>`;
                }).join('');

                // Fermer le conteneur de liste
                html += `</div>`;
            }
...
function updateThemeColorMeta() {
    ...
    // NOUVEAU: Couleurs par défaut améliorées
    let themeColor = isDark ? '#121212' : '#fdfdfd'; // Presque blanc pour le thème clair
    let navColor = isDark ? '#121212' : '#fdfdfd';
    ...
    if (document.body.classList.contains('theme-playstation')) {
        ...
        themeColor = '#3a4149'; // Couleur du haut du header
        navColor = '#0f1113';   /* Correspond au bas du rack (gradient) */
        ...
        themeColor = '#cceeff'; // Couleur du haut du dégradé de fond
        navColor = '#d9f1ff'; // Couleur du haut du dégradé de la nav
    } else if (document.body.classList.contains('theme-ds')) {
        themeColor = '#e2e8f0'; /* Gris clair DS */
        navColor = '#ffffff';   /* Bas blanc DS */
...
function openOverlay(overlay, sound, keepMusicPlaying = false) {
    ...
    // NOUVEAU: Marquer comme lu à l'ouverture des notifications pour que le point disparaisse
    if (overlay.id === 'notifications-overlay') {
        const allMessageIds = [
...
// NOUVEAU: Fonction globale pour fermer TOUS les overlays proprement
window.closeAllOverlays = function () {
    ...
    activeOverlay = null;
    const bodyTutorialClass = 'tutorial-active-body';
    document.body.classList.remove(bodyTutorialClass);
    document.documentElement.classList.remove(bodyTutorialClass);
};
...
function renderCards(containerId, cardsData, cardType = 'generic', animate = true) {
    ...
    Object.entries(cardsData).forEach(([key, item], index) => {
        ...
        // NOUVEAU: Les titres/vidéos/albums verrouillés sont STRICTEMENT CACHÉS des vues standards.
        // Ils n'apparaissent que dans le Shop.
        if (isLocked) return;
        ...
        const activeProfile = getActiveProfile(); // CORRECTION: Utilise la fonction helper pour la compatibilité mobile
        ...
        if (isListView && (cardType === 'title' || cardType === 'video' || cardType === 'search')) { // If it's a list view and a playable item
            ...
            card.innerHTML = `
                        ...
                        <div class="list-view-title" style="display: flex; flex-direction: column; justify-content: center;">
                            <span class="font-medium text-sm sm:text-base text-text-color">${translatedTitle}</span>
                            <span class="text-xs opacity-60">${isLocked ? getTranslation('buyToUnlock') : artistName}</span>
                            ${isAlbumLocked ? `
                        ...
                        <div class="tags-column text-sub-text-color text-xs items-center min-w-0 list-view-tags-col pc-only" style="display: flex; align-items: center; justify-content: flex-start;">
                            ${(item.tags && item.tags.length > 0) ? `<span class="tag-item" data-action="filter-tag" data-tag="${item.tags[0]}">${item.tags[0]}</span>` : ''}
                        </div>
                        <div class="tags-column text-sub-text-color text-xs items-center min-w-0 list-view-tags-col pc-only" style="display: flex; align-items: center; justify-content: flex-start;">
                            ${(item.tags && item.tags.length > 1) ? `<span class="tag-item" data-action="filter-tag" data-tag="${item.tags[1]}">${item.tags[1]}</span>` : ''}
                        </div>
            ...
            card.addEventListener('click', (e) => {
                ...
                if (menuBtn) {
                    ...
                    // CORRECTION: On vérifie s'il s'agit d'un titre musical ou d'une vidéo/bonus
                    if (isMusicTitle(item)) {
                        renderMusicTitleDetails(item);
                        showSection('music-title-details-section');
                        currentViewContext = { type: 'music-details', data: item.id };
                    } else {
                        // S'il s'agit d'une vidéo dans la liste, on l'ouvre dans le lecteur vidéo
                        showSection('large-player-section');
                        playVideoWhenReady(item, [], -1, 'video', false, false, false, true);
                    }
                }
            });
            ...
            switch (cardType) {
                ...
                case 'album':
                    ...
                    break;
                case 'video':
                    // NOUVEAU: Affiche le nom de l'album si disponible
                    // CORRECTION: Vérification de sécurité pour éviter les crashs si l'album n'est pas trouvé
                    if (item.albumId && siteData.contentData[activeProfile] && siteData.contentData[activeProfile].albums && siteData.contentData[activeProfile].albums[item.albumId]) {
                        description = siteData.contentData[activeProfile].albums[item.albumId].title;
                    } else {
                        description = getTranslation('videoLabel');
                    }
                    break;
...
function renderMusicTitleDetails(item, shouldPlay = false, isAutoTransition = false, sourceName = "") {
    ...
    const activeProfile = getActiveProfile();
    // CORRECTION: Utiliser item.profile (injecté lors du loadData) pour trouver le bon album, 
    // sinon la page de détails ne se charge pas si l'item n'est pas dans le profil actif.
    const itemProfile = item.profile || activeProfile;
    const album = siteData.contentData[itemProfile].albums[item.albumId];
...
function updateMp3PlayerInfo(item) {
    if (item && item.title) {
        ...
        if (mobilePlayerTitle && mobilePlayerTitle.querySelector('span')) {
            mobilePlayerTitle.querySelector('span').textContent = item.title;
            // NOUVEAU: Petit délai pour s'assurer que le texte est bien rendu avant de mesurer
            setTimeout(() => checkTitleOverflow(mobilePlayerTitle.querySelector('span')), 50);
        }

        if (mobilePlayerArtist && mobilePlayerArtist.querySelector('span')) {
            const activeProfileData = siteData.contentData[getActiveProfile()];
            ...
            setTimeout(() => checkTitleOverflow(mobilePlayerArtist.querySelector('span')), 50);
        }
...
function updateProgressBar() {
    ...
    if (duration > 0) { // If duration is valid
        ...
        // NOUVEAU: Mettre à jour la barre de progression mobile (Toujours, même si progress == 0)
...
function showSection(sectionId, updateHistory = true, previousContext = null) {
    ...
    const activeProfile = getActiveProfile();
    if (activeProfile) {
        renderSidebarNav(activeProfile, sectionId); // CORRECTION: Passe l'ID de la section active
    }
}
...
function updateVisibleCards(customTitle = null, animate = true) {
    ...
    // NOUVEAU: Mise à jour des badges de filtre (Desktop & Mobile)
    const updateBadge = (id) => {
        const badge = document.getElementById(id);
        if (badge) {
            badge.textContent = checkedTags.length > 0 ? checkedTags.length : '';
            badge.classList.toggle('hidden', checkedTags.length === 0);
        }
    };
    updateBadge('tags-filter-count');
    updateBadge('mobile-tags-filter-count');

    // NOUVEAU: Plus de "pills" dans la barre de recherche mobile
    const mobileSearchTagsContainer = document.getElementById('mobile-search-tags-container');
    if (mobileSearchTagsContainer) {
        mobileSearchTagsContainer.innerHTML = '';
        const mobileInput = document.getElementById('mobile-search-input');
        if (mobileInput) {
            mobileInput.style.paddingLeft = ''; // Reset padding
        }
    }
    ...
    const filteredResults = Object.fromEntries(Object.entries(allSearchableItems).filter(([id, item]) => {
        ...
        // NOUVEAU: On cache les items verrouillés des résultats de recherche hors shop
        const isLocked = item.isUnlockable && !unlockedDreamSeasonTracks.includes(id);

        return (titleMatch || tagTextMatch || albumTitleMatch) && tagsMatch && !isLocked;
...
function applyTheme(themeName) {
    ...
    if (isBgTheme) {
        ...
        // NOUVEAU: Si on applique un thème spécial, on force le mode clair pour éviter les conflits
        if (!isDefaultTheme) {
            document.body.classList.remove('dark-theme');
            if (themeSwitch) themeSwitch.checked = false;
            safeStorage.setItem('mmg-theme', 'light');
        }

        if (themeSwitch) {
            themeSwitch.disabled = !isDefaultTheme;
            themeSwitch.parentElement.classList.toggle('disabled-state', !isDefaultTheme);
        }
...
function renderSidebarNav(activeProfile, currentSectionId = null) {
    ...
    navContainer.innerHTML = homeLink + profileLinks + shopLink + libraryLink;
...
function setupTitleScrollObserver(containerSelector) {
    // NOUVEAU: On ne disconnect plus l'ancien observer car on veut pouvoir observer plusieurs sections en même temps.
    // L'observer est maintenant persistant.
    if (!window.titleScrollObserver) {
        const options = { root: document.querySelector('.content-section-board'), rootMargin: '0px', threshold: 0.8 };
        window.titleScrollObserver = new IntersectionObserver((entries) => {
    ...
    if (!window.titleResizeObserver) {
        window.titleResizeObserver = new ResizeObserver(entries => {
    ...
    titlesToObserve.forEach(title => {
        window.titleScrollObserver.observe(title);
        window.titleResizeObserver.observe(title);
    });
}
...
function renderDailyBonusProgress() {
    ...
    // NOUVEAU: On utilise toujours le design "Widget" (List Item) pour le bonus, sur PC comme sur Mobile
    ...
    const titleText = isClaimed ? getTranslation('bonusClaimed') || 'Bonus récupéré' : getTranslation('dailyBonusTitle');
    const subtitleText = isClaimed ? '' : `${getTranslation('claim')} +${loginStreak >= 7 ? getTranslation('bonus') : loginStreak} ${getTranslation('dailyBonusCoins')}`;
...
/**
 * NOUVEAU: Crée les boutons de détachement et initialise l'état flottant des panneaux.
 */
function initializeFloatingPanels() {
    const sidebar = document.querySelector('.sidebar');
    const player = document.querySelector('.bottom-ui-container');

    if (sidebar) {
        // Créer le conteneur pour le header de la sidebar
        const headerActions = document.createElement('div');
        headerActions.className = 'sidebar-header-actions';

        // Déplacer le brand-block existant dans le nouveau header
        const brandBlock = document.querySelector('.brand-block');
        if (brandBlock) {
            brandBlock.classList.add('sidebar-drag-handle'); // Poignée de déplacement
            headerActions.appendChild(brandBlock);
        }

        // Créer et ajouter le bouton de détachement
        const undockSidebarBtn = document.createElement('button');
        undockSidebarBtn.id = 'undock-sidebar-btn';
        undockSidebarBtn.className = 'undock-btn';
        undockSidebarBtn.title = "Détacher la sidebar";
        undockSidebarBtn.innerHTML = '<i class="fas fa-window-restore"></i>';
        headerActions.appendChild(undockSidebarBtn);

        // Insérer le nouveau header en haut du wrapper de contenu de la sidebar
        const sidebarContentWrapper = sidebar.querySelector('.sidebar-content-wrapper');
        if (sidebarContentWrapper) {
            sidebarContentWrapper.insertBefore(headerActions, sidebarContentWrapper.firstChild);
        }

        // Appliquer l'état flottant au chargement
        if (sidebarFloatingState.isFloating) {
            sidebar.classList.add('floating');
            sidebar.style.top = `${sidebarFloatingState.y}px`;
            sidebar.style.left = `${sidebarFloatingState.x}px`;
            sidebar.style.width = `${sidebarFloatingState.width}px`;
            sidebar.style.height = `${sidebarFloatingState.height}px`;
            makeDraggable(sidebar, brandBlock, 'mmg-sidebar-floating-state');
        }
    }

    if (player) {
        // Créer et ajouter le bouton de détachement pour le lecteur
        const undockPlayerBtn = document.createElement('i');
        undockPlayerBtn.id = 'undock-player-btn';
        undockPlayerBtn.className = 'fas fa-window-restore undock-btn';
        undockPlayerBtn.title = "Détacher le lecteur";

        const playerRightControls = player.querySelector('.player-right-controls');
        if (playerRightControls) {
            // Insérer avant le premier enfant pour le mettre à gauche des autres icônes
            playerRightControls.insertBefore(undockPlayerBtn, playerRightControls.firstChild);
        }

        // Appliquer l'état flottant au chargement
        if (playerFloatingState.isFloating) {
            document.body.classList.add('player-floating');
            player.classList.add('floating');
            player.style.top = `${playerFloatingState.y}px`;
            player.style.left = `${playerFloatingState.x}px`;
            makeDraggable(player, player, 'mmg-player-floating-state');
        }
    }
}
...
function initializeFloatingPanels() {
    ...
    if (sidebar) {
        // Créer le conteneur pour le header de la sidebar s'il n'existe pas
        let headerActions = sidebar.querySelector('.sidebar-header-actions');
        if (!headerActions) {
            ...
            if (brandBlock && sidebarContentWrapper) {
                brandBlock.classList.add('sidebar-drag-handle'); // Poignée de déplacement
                headerActions.appendChild(brandBlock);
                ...
                undockSidebarBtn.id = 'undock-sidebar-btn';
                undockSidebarBtn.className = 'undock-btn';
                undockSidebarBtn.title = "Détacher/Ancrer la sidebar";
                undockSidebarBtn.innerHTML = '<i class="fas fa-window-restore"></i>';

                // Ajouter l'écouteur d'événement ici
                undockSidebarBtn.addEventListener('click', () => {
                    sidebar.classList.toggle('floating');
                    sidebarFloatingState.isFloating = sidebar.classList.contains('floating');
                    ...
                    if (sidebarFloatingState.isFloating) {
                        ...
                        sidebar.style = ''; // Reset styles
                    }
                    safeStorage.setItem('mmg-sidebar-floating-state', JSON.stringify(sidebarFloatingState));
                });
        ...
        if (sidebarFloatingState.isFloating) {
            ...
            if (brandBlock) makeDraggable(sidebar, brandBlock, 'mmg-sidebar-floating-state');
        }
    }

    if (player) {
        const playerRightControls = player.querySelector('.player-right-controls');
        if (playerRightControls && !document.getElementById('undock-player-btn')) {
            const undockPlayerBtn = document.createElement('i');
            undockPlayerBtn.id = 'undock-player-btn';
            undockPlayerBtn.className = 'fas fa-window-restore undock-btn clickable-icon';
            undockPlayerBtn.title = "Détacher/Ancrer le lecteur";

            undockPlayerBtn.addEventListener('click', () => {
                document.body.classList.toggle('player-floating');
                player.classList.toggle('floating');
                playerFloatingState.isFloating = player.classList.contains('floating');
                ...
                if (playerFloatingState.isFloating) {
                    ...
                    player.style = ''; // Reset styles
                }
                safeStorage.setItem('mmg-player-floating-state', JSON.stringify(playerFloatingState));
            });
...
function initializeApp() {
    ...
    // NOUVEAU: Initialiser les panneaux flottants
    ...
    if ('serviceWorker' in navigator) {
            ...
            registration.addEventListener('updatefound', () => {
                ...
                newWorker.addEventListener('statechange', () => {
                    if (newWorker.state === 'installed' && navigator.serviceWorker.controller) {
                        // Une nouvelle version est disponible et installée, on notifie l'utilisateur
        ...
        // NOUVEAU: Recharger la page quand le nouveau SW prend le contrôle
        let refreshing;
...
// NOUVEAU: Fonction centralisée pour changer de profil
function switchProfile(targetProfile) {
    const previousProfile = getActiveProfile();
    if (previousProfile === targetProfile) return;

    // 1. Mise à jour du Switch Mobile
    document.querySelectorAll('#mobile-profile-switch .profile-switch-btn').forEach(btn => {
        btn.classList.toggle('active', btn.dataset.project === targetProfile);
    });

    // 2. Mise à jour du Branding (Studio)
    const mmgBranding = document.getElementById('mmg-branding');
    if (mmgBranding) {
        mmgBranding.classList.toggle('is-music', targetProfile === 'mmg-music');
        mmgBranding.classList.toggle('is-beats', targetProfile === 'mmg-beats');
    }

    // 3. Mise à jour du Switch Spotimon
    const spotimonSwitch = document.getElementById('spotimon-profile-switch');
    if (spotimonSwitch) {
        spotimonSwitch.classList.toggle('is-beats', targetProfile === 'mmg-beats');
    }

    // 4. Mise à jour du bouton "À propos"
    const aboutSwitchBtn = document.getElementById('about-switch-profile-btn');
    if (aboutSwitchBtn) {
        const otherProfileName = targetProfile === 'mmg-music' ? 'Beats' : 'Music';
        const switchText = aboutSwitchBtn.querySelector('.switch-text');
        if (switchText) switchText.textContent = `Switch to ${otherProfileName}`;
    }

    // 5. Mise à jour du titre de la carte À propos
    const aboutTitle = document.querySelector('.about-card-title');
    if (aboutTitle) {
        aboutTitle.textContent = targetProfile === 'mmg-music' ? 'Mmg Music' : 'Mmg Beats';
    }

    // Mise à jour de l'indicateur visuel (barre sous l'onglet)
    setTimeout(() => {
        const container = document.querySelector('.profile-switch');
        if (container) updateSlidingIndicator(container);
    }, 0);

    // Logique de navigation intelligente (rester sur la même page si possible)
    const currentSectionId = document.querySelector('.page-section:not(.hidden)')?.id || 'home-dashboard-section';
    let sectionToLoad = 'home-dashboard-section';
    const commonSections = ['library-section', 'shop-section'];

    if (commonSections.includes(currentSectionId)) {
        sectionToLoad = currentSectionId;
    } else if (currentSectionId !== 'home-dashboard-section') {
        if (siteData.projectData && siteData.projectData[previousProfile]) {
            const prevItem = Object.values(siteData.projectData[previousProfile]).find(p => p.link === currentSectionId);
            if (prevItem) {
                const equivalentSection = Object.values(siteData.projectData[targetProfile]).find(item => {
    ...
    renderSidebarNav(targetProfile);
...
function setupEventListeners() {
    ...
    // (Les écouteurs sont regroupés plus bas dans la fonction)
    ...
    if (aboutSwitchBtn) {
        aboutSwitchBtn.addEventListener('click', (e) => {
            e.preventDefault();
            const activeProfile = getActiveProfile();
            const targetProfile = activeProfile === 'mmg-music' ? 'mmg-beats' : 'mmg-music';
            switchProfile(targetProfile);
    ...
    // NOUVEAU: Fermer la recherche au clic extérieur (Global Listener comme dans FinalUI)
    document.addEventListener('click', (e) => {
        const topBar = document.querySelector('.top-bar');
        if (topBar && topBar.classList.contains('searching') && !topBar.contains(e.target)) {
    ...
    // NOUVEAU: Setup listeners for the new mobile search (Style FinalUI)
    const mobileSearchBtn = document.getElementById('mobile-search-btn');
    const mobileSearchCancelBtn = document.getElementById('mobile-search-cancel-btn');
    const mobileSearchTagBtn = document.getElementById('mobile-search-tag-btn');
    ...
    if (mobileSearchBtn) {
        ...
        mobileSearchBtn.addEventListener('touchstart', triggerSearch, { passive: false });
    }

    if (mobileSearchTagBtn) {
        mobileSearchTagBtn.addEventListener('click', (e) => {
            e.preventDefault();
            openOverlay(document.getElementById('tags-filter-overlay'), sounds.select, true);
        });
    }

    if (mobileSearchCancelBtn) {
        mobileSearchCancelBtn.addEventListener('click', (e) => {
            e.preventDefault();
            toggleSearch();
        });
    }

    // NOUVEAU: Listener pour l'input de recherche mobile (SYNCHRONISATION)
    ...
    mainWrapper.addEventListener('click', (e) => { // Add click listener to main wrapper
        ...
        if (youtubeId) {
            ...
            const activeProfile = getActiveProfile();
            // CORRECTION: renderSidebarNav est déjà appelé par handleMenuNavigation -> showSection
            // renderSidebarNav(activeProfile); 
            ...
            // NOUVEAU: Si c'est un album vidéo, on navigue vers la liste des vidéos
            const card = e.target.closest('.card');
            if (card && card.parentElement && card.parentElement.id === 'video-albums-cards') {
                handleMenuNavigation('video-list-section', true, { type: 'video-list', data: albumId });
                return;
            }
    ...
    mainWrapper.addEventListener('touchstart', (e) => {
        ...
        if (card) { // If card exists
            ...
            // NOUVEAU: Gestion du clic sur un album vidéo
            if (card.parentElement.id === 'video-albums-cards') {
                e.preventDefault();
                e.stopPropagation();
                const albumId = card.dataset.itemId;
                playAudio(sounds.select);
                handleMenuNavigation('video-list-section', true, { type: 'video-list', data: albumId });
                return;
            }
        }
    }, { passive: false }); // CORRECTION: passive: false pour permettre e.preventDefault()
    ...
    document.getElementById('cheat-code-btn').addEventListener('click', () => {
        ...
        if (code === 'gameshark') { // If cheat code is gameshark
            ...
            showUpdateNotification(mockRegistration);
            input.value = '';
        }
        else {
    ...
    // NOUVEAU: Écouteurs pour les panneaux flottants
    const sidebar = document.querySelector('.sidebar');
    const player = document.querySelector('.bottom-ui-container');
    const undockSidebarBtn = document.getElementById('undock-sidebar-btn');
    const undockPlayerBtn = document.getElementById('undock-player-btn');

    if (undockSidebarBtn && sidebar) {
        undockSidebarBtn.addEventListener('click', () => {
            sidebar.classList.toggle('floating');
            sidebarFloatingState.isFloating = sidebar.classList.contains('floating');

            if (sidebarFloatingState.isFloating) {
                // Position initiale si on détache pour la première fois
                sidebar.style.top = `${sidebarFloatingState.y}px`;
                sidebar.style.left = `${sidebarFloatingState.x}px`;
                sidebar.style.width = `${sidebarFloatingState.width}px`;
                sidebar.style.height = `${sidebarFloatingState.height}px`;
                makeDraggable(sidebar, sidebar.querySelector('.sidebar-drag-handle'), 'mmg-sidebar-floating-state');
            } else {
                // Réinitialiser les styles inline pour que le CSS reprenne le contrôle
                sidebar.style.top = '';
                sidebar.style.left = '';
                sidebar.style.width = '';
        ...
        // Sauvegarder la taille lors du redimensionnement
        new ResizeObserver(() => {
            if (sidebar.classList.contains('floating')) {
                sidebarFloatingState.width = sidebar.offsetWidth;
                sidebarFloatingState.height = sidebar.offsetHeight;
                safeStorage.setItem('mmg-sidebar-floating-state', JSON.stringify(sidebarFloatingState));
            }
        }).observe(sidebar);
    }

    if (undockPlayerBtn && player) {
        undockPlayerBtn.addEventListener('click', () => {
            document.body.classList.toggle('player-floating');
            player.classList.toggle('floating');
            playerFloatingState.isFloating = player.classList.contains('floating');
    ...
    miniPlayer.addEventListener('click', (e) => {
        ...
        if (miniPlayer.classList.contains('bubble-mode')) {
            ...
            if (isMusicTitle(currentPlayingItem)) {
                ...
                playAudio(sounds.maximize);
                // Forcer le recalcul du défilement
                setTimeout(() => {
                    const t = document.getElementById('mobile-player-title')?.querySelector('span');
                    const a = document.getElementById('mobile-player-artist')?.querySelector('span');
                    if (t) checkTitleOverflow(t);
                    if (a) checkTitleOverflow(a);
                }, 400);
            } else {
                showSection('large-player-section');
            }
    ...
    document.querySelectorAll('.settings-card').forEach(panel => {
        ...
        panel.addEventListener('touchend', e => {
            ...
            if (panelTouchStartY > 0 && touchEndY > panelTouchStartY + 75) { // Swipe vers le bas de 75px
                closeOverlay(); // Utilise la fonction centralisée qui joue déjà sounds.back
            }
    ...
    if (themesContainer) {
        themesContainer.addEventListener('click', (e) => {
            const lockedThemeCard = e.target.closest('.theme-row-premium.locked, .shop-list-item.locked');
    ...
    // NOUVEAU: Gestionnaire pour les boutons de changement de vue (Grille/Liste)
    document.querySelectorAll('.view-switch-btn').forEach(btn => {
        btn.addEventListener('click', (e) => {
            const view = e.currentTarget.dataset.view;
            if (view) {
                playAudio(sounds.select);
                localStorage.setItem('mmg-global-view', view);
                updateViewSwitcherUI();
                refreshCurrentView();
            }
        });
    ...
    if (buyBtn || (actionTarget && actionTarget.classList.contains('shop-buy-btn')) || isBuyable) {
        ...
        if (trackItem && trackItem.isUnlockable) {
            ...
            if (userCoins >= COIN_COST_UNLOCK) {
                ...
                playAudio(sounds.coin);
                showDialog(`${getTranslation('youUnlocked')} "${trackItem.title}"!`);
                updateCoinDisplay();
...
function unlockCardUI(itemId, isShopItem = false) {
    ...
    unlockedCards.forEach(card => {
        ...
        if (isShopItem) {
            ...
            // NOUVEAU: Mise à jour visuelle pour le mode liste
            const iconContainer = card.querySelector('.fa-chevron-right');
            if (iconContainer) {
                const circle = document.createElement('div');
                circle.style.cssText = "width: 20px; height: 20px; border: 2px solid var(--text-muted); border-radius: 50%;";
                iconContainer.replaceWith(circle);
            }

            const subtitle = card.querySelector('.shop-list-subtitle');
            if (subtitle) {
                subtitle.innerHTML = '<span>Débloqué</span>';
            }
...
function updateMobileHeader(sectionId) {
    ...
    const staticSectionTitles = {
        ...
        'video-list-section': getTranslation('videos'), // NOUVEAU
...
window.renderVideosPage = () => {
    const container = document.getElementById('video-albums-cards');
    if (!container) return;

    const activeProfile = getActiveProfile();
    const profileContent = siteData.contentData[activeProfile];
    const albums = profileContent.albums;

    // NOUVEAU: Ne garde que les albums qui ont au moins une vidéo débloquée
    const videoAlbums = Object.fromEntries(
        Object.entries(albums).filter(([albumId, _]) => {
            const hasUnlockedVideos = Object.entries(profileContent.videos).some(([vidId, v]) => v.albumId === albumId && (!v.isUnlockable || unlockedDreamSeasonTracks.includes(vidId)));
            const hasUnlockedBonus = Object.entries(profileContent.bonus).some(([vidId, v]) => v.albumId === albumId && (!v.isUnlockable || unlockedDreamSeasonTracks.includes(vidId)));
            return hasUnlockedVideos || hasUnlockedBonus;
        }).map(([id, album]) => {
            const modifiedAlbum = { ...album };
            delete modifiedAlbum.langKey; // On retire la clé de traduction pour forcer le titre modifié
            modifiedAlbum.title = `${album.title} (${getTranslation('videos')})`;
            return [id, modifiedAlbum];
        })
    );

    renderCards('video-albums-cards', videoAlbums, 'album');
    currentViewContext = { type: 'videos', data: null };
};
...
function handleMenuNavigation(dataLink, updateHistory = true, context = null) {
    ...
    // CORRECTION: Filtrer les albums pour ne montrer que ceux qui ont au moins un titre débloqué
    const filteredAlbums = Object.fromEntries(
        Object.entries(getSectionData('albums-section') || profileContent.albums).filter(([albumId, _]) => {
            const titles = siteData.contentData[activeProfile].titles;
            return Object.values(titles).some(t => t.albumId === albumId && (!t.isUnlockable || unlockedDreamSeasonTracks.includes(t.id)));
        })
    );

    const sections = {
        'albums-section': { data: filteredAlbums, type: 'album', container: 'albums-cards' }
    };
    ...
    if (dataLink === 'home-dashboard-section') {
        ...
        const activeShopTab = (effectiveContext && effectiveContext.type === 'shop' && effectiveContext.data) ? effectiveContext.data : 'themes';
        currentViewContext = { type: 'shop', data: activeShopTab };
        renderShopPage(activeShopTab);
    } else if (dataLink === 'videos-section') {
        // CORRECTION: Gère la navigation vers la section vidéo unifiée.
        renderVideosPage();
    } else if (dataLink === 'video-list-section') {
        // NOUVEAU: Gère la navigation vers la liste des vidéos d'un album
        currentViewContext = effectiveContext;
        if (effectiveContext && effectiveContext.type === 'video-list' && effectiveContext.data) {
            const albumId = effectiveContext.data;
            const activeProfile = getActiveProfile();
            const profileContent = siteData.contentData[activeProfile];

            // Récupère toutes les vidéos et bonus de cet album (uniquement débloqués car le reste est au shop)
            const albumVideos = {
                ...Object.fromEntries(Object.entries(profileContent.videos).filter(([id, v]) => {
                    const isLocked = v.isUnlockable && !unlockedDreamSeasonTracks.includes(id);
                    return v.albumId === albumId && !isLocked;
                })),
                ...Object.fromEntries(Object.entries(profileContent.bonus).filter(([id, v]) => {
                    const isLocked = v.isUnlockable && !unlockedDreamSeasonTracks.includes(id);
                    return v.albumId === albumId && !isLocked;
                }))
            };

            // MODIFICATION: Ajout de "(Vidéos)" dans le titre de la page de liste
            const albumTitle = profileContent.albums[albumId]?.title || getTranslation('videos');
            document.getElementById('video-list-title').textContent = `${albumTitle} (${getTranslation('videos')})`;
            renderCards('video-list-cards', albumVideos, 'video');

            // La logique de défilement est déjà gérée par renderCards via setupTitleScrollObserver
        }
        ...
        if (effectiveContext && effectiveContext.type === 'titles' && effectiveContext.data) {
            const albumId = effectiveContext.data;
            const titlesForAlbum = Object.fromEntries(Object.entries(siteData.contentData[activeProfile].titles).filter(([id, title]) => {
                const isLocked = title.isUnlockable && !unlockedDreamSeasonTracks.includes(id);
                return title.albumId === albumId && !isLocked;
            }));
            renderCards('titles-cards', titlesForAlbum, 'title');
            ...
            const titlesForAlbum = Object.fromEntries(Object.entries(siteData.contentData[activeProfile].titles).filter(([id, title]) => {
                const isLocked = title.isUnlockable && !unlockedDreamSeasonTracks.includes(id);
                return title.albumId === defaultAlbumId && !isLocked;
            }));
            renderCards('titles-cards', titlesForAlbum, 'title');
...
function renderLibraryPage(activeTabId = 'liked') {
    ...
    if (activeTabId === 'liked') {
        itemsToShow = [...likedSongs].map(id => findItemById(id)).filter(item => {
            ...
            // NOUVEAU: Cacher les titres verrouillés de la bibliothèque également
        ...
        itemsToShow = currentPlaylist.map(id => findItemById(id)).filter(item => {
            if (!item) return false;
            // NOUVEAU: On autorise la lecture si c'est déjà dans la playlist de l'utilisateur
            // (Mais on pourrait filtrer si on veut une règle stricte)
            return true;
        });
        ...
        // NOUVEAU: Gère les playlists sauvegardées et recommandées
    ...
    container.innerHTML = recommendedPlaylists.map(playlist => {
        return `
            <div class="reco-card" data-item-id="${playlist.id}">
                <div class="reco-image-container ${playlist.cssClass}">
...
const newsData = {
    'lancement': {
        ...
        content: `Bienvenue sur la version finale de Mmg Studio ! Nous avons repensé l'expérience pour qu'elle soit unique sur chaque plateforme.<br><br><strong>🖥️ Sur Ordinateur : Le Tableau de Bord Ultime</strong><br>Profitez d'un espace de travail musical complet conçu pour la productivité et la découverte.<br>• <strong>Interface Modulaire :</strong> Un dashboard avec widgets dynamiques (Bonus quotidien, Nouveautés, Playlists).<br>• <strong>Lecteur Ancrable :</strong> Le lecteur audio peut être fixé en bas ou détaché en fenêtre flottante.<br>• <strong>Gestion Avancée :</strong> Créez vos playlists et gérez votre file d'attente par simple glisser-déposer.<br>• <strong>Immersion :</strong> Des thèmes visuels haute résolution (Music Studio, PS3, Aero) qui transforment votre navigateur.<br><br><strong>📱 Sur Mobile : Une App dans votre Poche</strong><br>Installez le site comme une application native (PWA) pour une fluidité parfaite.<br>• <strong>Navigation Gestuelle :</strong> Swipez pour revenir en arrière, fermer le lecteur ou changer d'onglet.<br>• <strong>Lecteur Hybride :</strong> Passez du mini-lecteur au mode plein écran, ou réduisez-le en "Bulle" flottante pour naviguer librement.<br>• <strong>Thèmes Exclusifs :</strong> Débloquez le thème "Dual Screen" qui transforme votre mobile en console portable rétro.<br>• <strong>Mode Hors-Ligne :</strong> Une interface réactive qui s'adapte à votre orientation (Portrait/Paysage).<br><br>Merci de faire partie de l'aventure !`
    },
    'studio': {
...
function renderNewsDetailsPage(newsId) {
    ...
    // NOUVEAU: Met à jour le titre de la barre du haut
    const topBarTitle = document.getElementById('mobile-header-title');
    if (topBarTitle) topBarTitle.innerText = "News";

    container.innerHTML = `
        <div class="music-details-grid">
            <div class="music-details-container">
                <div class="music-art-panel" style="height: auto; max-height: none; position: relative; top: 0; padding-top: 0;">
                    <div class="album-art-wrapper" style="margin-top: 0;">
                        <img src="${news.image}" alt="${news.title}" class="news-main-img" style="width: 100%; border-radius: 24px; box-shadow: 0 20px 40px rgba(0,0,0,0.2);">
                    </div>
                </div>
                <div class="music-info-panel">
                    <div class="details-title-wrapper">
                        <h2 class="news-details-title">${news.title}</h2>
                    </div>
                    <div class="details-meta-container" style="margin-bottom: 20px;">
                        <span class="tag-link" style="color: var(--active-color); font-weight: 800; text-transform: uppercase; letter-spacing: 0.1em;">
            ...
            <!-- Colonne 3 (Vidéos) vide pour l'équilibre du layout sur PC -->
            <div class="associated-videos-panel" style="visibility: hidden;"></div>
        </div>
    `;
}
...
function renderUpcomingReleasePage() {
    ...
    // NOUVEAU: Met à jour le titre de la barre du haut
    const topBarTitle = document.getElementById('mobile-header-title');
    if (topBarTitle) topBarTitle.innerText = "Upcoming Release";
    ...
    container.innerHTML = `
        <div class="music-details-grid">
            <div class="music-details-container">
                <div class="music-art-panel" style="height: auto; max-height: none; position: relative; top: 0; padding-top: 0;">
                    <div class="album-art-wrapper" style="margin-top: 0;">
                        <img src="${upcomingData.image}" alt="${upcomingData.title}" class="news-main-img" style="width: 100%; border-radius: 24px; box-shadow: 0 20px 40px rgba(0,0,0,0.2);">
                    </div>
                </div>
                <div class="music-info-panel">
                    <div class="details-title-wrapper">
                        <h2 class="news-details-title">${upcomingData.title}</h2>
                    </div>
                    <div class="details-meta-container" style="margin-bottom: 20px;">
                        <span class="tag-link" style="color: var(--active-color); font-weight: 800; text-transform: uppercase; letter-spacing: 0.1em;">
                    ...
                    <div class="news-details-description">
                        ...
                        <div style="margin-top: 30px;">
                             <button class="daily-bonus-action-btn" disabled style="width: auto; padding: 10px 20px; font-size: 0.9rem;">${getTranslation('comingSoon')}</button>
                        </div>
                    </div>
                </div>
            </div>
...
function renderUpcomingMiniWidget() {
    const container = document.getElementById('upcoming-mini-widget');
    if (!container) return;

    // On utilise les mêmes données que pour l'accordion ou la page dédiée
    const upcomingTitle = "Super Mmg Bros (Desert)";
    const upcomingImage = "assets/pochettes/Pochette-SuperMmgBros(Desert).webp";

    container.innerHTML = `
        <div class="upcoming-mini-card clickable-upcoming" style="height: 100%; cursor: pointer;" data-link="upcoming-release-section">
            <img src="${upcomingImage}" class="upcoming-mini-img" alt="${upcomingTitle}">
            <div class="upcoming-mini-info">
                <p class="upcoming-mini-subtitle" style="font-size: 0.65rem; text-transform: uppercase; color: var(--active-color); font-weight: 800; letter-spacing: 0.1em; margin-bottom: 2px;">Upcoming Release</p>
                <h4 class="upcoming-mini-title">${upcomingTitle}</h4>
                <div style="background: white; color: black; padding: 4px 10px; border-radius: 20px; font-size: 0.6rem; font-weight: 800; display: inline-block;">AVRIL 2026</div>
            </div>
        </div>
    `;

    // Événement de clic pour rediriger
    const card = container.querySelector('.upcoming-mini-card');
    if (card) {
        card.onclick = () => showPage('upcoming-release-section');
    }
}

// --- NOUVEAUX RENDERS POUR LE DESIGN PC (SPOTLIGHT) ---



function activateHeroItem(element) {
    const items = document.querySelectorAll('.hero-item');
    items.forEach(item => item.classList.remove('active'));
    element.classList.add('active');
}

// --- RENDERS POUR LE DESIGN PC (BIG SPOTLIGHT) ---

function renderPCFeaturedSpotlight() {
    const container = document.getElementById('pc-featured-spotlight');
    if (!container) return;
    ...
    container.innerHTML = `
        <div class="spotlight-featured-inner">
            <div class="spotlight-featured-image">
                <img src="${getCorrectImagePath(latestItem, 'full')}" alt="${latestItem.title}">
            </div>
            <div class="spotlight-featured-info">
                <span class="featured-label">${getTranslation('lastRelease')}</span>
                <h1 class="featured-title">${latestItem.title}</h1>
                <p class="featured-desc">Découvrez la dernière production signée ${activeProfile === 'mmg-music' ? 'Mmg' : 'Mmg Beats'}.</p>
                <button class="spotlight-btn dashboard-card-button" 
                        data-youtube-id="${latestItem.youtube_id}" 
                        data-item-id="${latestItem.id}"
                        style="padding: 14px 30px; border-radius: 30px; font-weight: 900; background: var(--active-color); color: white; border: none; font-size: 1rem;">
                    <i class="fas fa-play" style="margin-right: 10px;"></i> ${getTranslation('listenNow')}
                </button>
            </div>
        </div>
    `;
...
function renderPCUpcomingSpotlight() {
    const container = document.getElementById('pc-upcoming-spotlight');
    if (!container) return;
    ...
    container.innerHTML = `
        <div class="spotlight-upcoming-inner">
            <img src="${upcomingImage}" class="upcoming-bg-img" alt="${upcomingTitle}">
            <div class="upcoming-content-overlay">
                <span class="upcoming-badge">Coming Soon</span>
                <h4 class="upcoming-title-pc">${upcomingTitle}</h4>
                <div class="upcoming-actions">
                    <button class="upcoming-btn-preview" id="upcoming-preview-btn">
                        <i class="fas fa-volume-up" style="margin-right: 8px;"></i> Court Extrait
                    </button>
                    <button class="upcoming-btn-preview" style="background: transparent;" onclick="showPage('upcoming-release-section')">
                        Détails
                    </button>
                </div>
            </div>
    ...
    // Gestion de l'extrait audio (Mock)
    const previewBtn = container.querySelector('#upcoming-preview-btn');
    if (previewBtn) {
        previewBtn.onclick = (e) => {
            e.stopPropagation();
            // On joue un petit son ou on affiche un toast pour l'instant
            if (typeof playAudio === 'function') playAudio(sounds.levelUp); // Mock
            showToast("Lecture de l'extrait en cours...");
        };
    }
}

function renderDashboard() { // Render dashboard sections
    renderHeroAccordion(); // Accordion unifiée (PC & Mobile)
    renderDailyBonusProgress(); // Widget Bonus
...
function startHeroAccordionAutoScroll() {
    ...
    heroAccordionInterval = setInterval(() => {
        // CORRECTION: Sécurité pour ne pas défiler (et faire du bruit) si la page est masquée ou si on n'est pas sur le dashboard
        ...
        if (activePanel && panels.length > 0) {
            ...
            if (nextPanel) {
                toggleAccordion(nextPanel.id, true); // true = silent
            }
        }
    }, 6000);
}
    ...
    container.innerHTML = `
        <div class="bonus-content">
            <div class="bonus-info">
                <i class="fas fa-gift" style="color: #db2777; margin-right: 8px;"></i>
                ${streakText} <span style="color: #cbd5e1; margin: 0 4px;">|</span> <span style="vertical-align: middle;">${rewardAmount}</span> ${COIN_SVG}
            </div>
        </div>
...
function handleRecoPlaylistClick(e) {
    ...
    // Check for save button click first (in list view)
    const saveBtn = e.target.closest('.reco-list-action');
    if (saveBtn) {
        const playlistName = saveBtn.dataset.playlistName;
        if (playlistName) {
    ...
    const titleEl = card.querySelector('.bank-title') || card.querySelector('.card__title span') || card.querySelector('.reco-title') || card.querySelector('.reco-list-title');
...
function renderRecommendedPlaylists() {
    ...
    // Translation keys
    const chillName = getTranslation('playlistChillName') || 'Chill Game Music';
    const chillSub = getTranslation('playlistChillSubtitle') || 'Sélection personnalisée';
    const frutigerName = getTranslation('playlistFrutigerName') || 'Frutiger Aero';
    const frutigerSub = getTranslation('playlistFrutigerSubtitle') || 'Beats instrumentaux';

    const recommendedPlaylists = [
        {
            id: 'chill',
            name: chillName,
            cssClass: 'reco-playlist-chill',
            subtitle: chillSub,
            icon: 'fa-sun' // Sun icon as requested
        },
        {
            id: 'frutiger',
            name: frutigerName,
            cssClass: 'reco-playlist-frutiger',
            subtitle: frutigerSub,
            icon: 'fa-microchip' // Technology icon as requested
        }
    ];

    container.innerHTML = recommendedPlaylists.map(playlist => {
        const isSaved = savedPlaylists[playlist.name] !== undefined;
        const actionIcon = isSaved ? 'fa-check' : 'fa-plus';

        // NOUVEAU: Style "List Item" type FinalUI avec gradients CSS
        return `
            <div class="reco-list-item" data-item-id="${playlist.id}">
                <div class="reco-list-img ${playlist.cssClass}">
                    <i class="fas ${playlist.icon}"></i>
                </div>
                <div class="reco-list-content">
                    <p class="reco-list-title">${playlist.name}</p>
                    <p class="reco-list-subtitle">${playlist.subtitle}</p>
                </div>
                <div class="reco-list-action" data-playlist-name="${playlist.name}">
                    <i class="fas ${actionIcon}" style="color: ${isSaved ? '#10b981' : 'var(--text-muted)'};"></i>
                </div>
            </div>`;
    }).join('');
}

// NOUVEAU: Gestion de la recherche mobile
// NOUVEAU: Gestion de la recherche mobile (Style FinalUI)
function toggleSearch() {
    const topBar = document.querySelector('.top-bar'); // .top-bar correspond à .mobile-header dans FinalUI
    if (!topBar) return;

    const isSearching = topBar.classList.toggle('searching');

    if (isSearching) {
        document.body.classList.add('scroll-locked'); // Bloquer le scroll
        const input = document.getElementById('mobile-search-input');
        if (input) {
            setTimeout(() => input.focus(), 50); // Petit délai pour l'animation
        }
        if (typeof lucide !== 'undefined') lucide.createIcons(); // Rafraîchir l'icône loupe interne
        ...
        const handleOutsideClick = (e) => {
            ...
            if (headerSearch && !headerSearch.contains(e.target) && (!searchBtn || !searchBtn.contains(e.target))) {
                if (topBar.classList.contains('searching')) {
                    toggleSearch();
                }
...
// Logic for mobile search listeners moved to setupEventListeners for reliability

function savePlaylist(playlistName) {
    if (savedPlaylists[playlistName]) {
        delete savedPlaylists[playlistName];
        safeStorage.setItem('mmg-savedPlaylists', JSON.stringify(savedPlaylists));
        // Use showDialog or similar feedback if showToast not available
        // Assuming showDialog exists based on previous code usage
    ...
    renderRecommendedPlaylists(); // Update UI
}
...
function renderUpdateLog() {
        ...
        dynamicNotificationsHtml += `
            <div class="notification-item unlock-prompt" onclick="handleMenuNavigation('shop-section'); showShopTab('tracks'); closeOverlay(null);">
                <div class="notification-details">
                    ...
                    <p style="font-size: 0.8em; opacity: 0.8;">${getTranslation('unlockTrackWithCoins', { COIN_COST_UNLOCK: COIN_COST_UNLOCK })}</p>
                </div>
                <i class="fas fa-chevron-right notification-arrow" style="font-size: 14px; opacity: 0.7;"></i>
            </div>
        `;
    }
    ...
    if (cheapestBackground && userCoins >= cheapestBackground.cost) {
        dynamicNotificationsHtml += `
            <div class="notification-item unlock-prompt" onclick="handleMenuNavigation('shop-section'); showShopTab('backgrounds'); closeOverlay(null);">
                <div class="notification-details">
                    <h4>${getTranslation('unlockAvailableTitle')}</h4>
                    <p style="font-size: 0.8em; opacity: 0.8;">${getTranslation('unlockBackgroundWithCoins', { cost: cheapestBackground.cost })}</p>
                </div>
                <i class="fas fa-chevron-right notification-arrow" style="font-size: 14px; opacity: 0.7;"></i>
            </div>
        `;
    ...
    // On s'assure que les notifications sans onclick explicite dans le template fonctionnent aussi si besoin via des listeners
...
// NOUVEAU: Affiche la notification de mise à jour
function showUpdateNotification(registration) {
    const notification = document.createElement('div');
    notification.className = 'update-notification';
    notification.innerHTML = `
        <div style="display:flex; flex-direction:column;">
            <span style="font-weight:800; font-size:1rem;">Mise à jour disponible</span>
            <span style="font-size:0.8rem; opacity:0.8;">Une nouvelle version du site est prête.</span>
        </div>
