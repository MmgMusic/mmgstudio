...
function onPlayerStateChange(event) {
    ...
    if (event.data === YT.PlayerState.ENDED) {
        ...
        if (miniPlayer && currentPlayingItem) miniPlayer.classList.remove('hidden');
...
function playVideoWhenReady(item, playlistIds = [], index = -1, playbackOriginType = 'titles', forceTempPlay = false, fromAutoplay = false, isSwipe = false) {
    ...
    // MODIFICATION: On force toujours l'affichage de la page de détails pour les titres musicaux.
...
function loadAndPlayVideo(item, fromAutoplay = false, isSwipe = false, cueOnly = false) {
    ...
    // MODIFICATION: On ne montre le mini-player immédiatement que si ce n'est PAS un cueOnly
    if (!cueOnly) {
        const miniPlayer = document.getElementById('mobile-mini-player');
...
function renderMusicTitleDetails(item) {
    ...
    loadAndPlayVideo(item, false, false, true); // true = cue only
...
function setupEventListeners() {
    ...
    document.getElementById('next-video-btn').addEventListener('click', () => playNextTrack(1, false));
    document.getElementById('prev-video-btn').addEventListener('click', () => playNextTrack(-1, false));
