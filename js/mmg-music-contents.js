...
function updateNotificationDot() {
    ...
    // MODIFICATION: Le point rouge apparaît uniquement si on peut débloquer quelque chose
    // ET que l'utilisateur n'a pas déjà "ignoré" cette notification (Option 2)
    const shouldShowDot = canUnlockSomething && (userCoins > coinsAtLastDismissal);
    ...
    // Mettre à jour l'état du bouton "Marquer comme lu" (il sert ici de bouton "Ignorer")
    const markAsReadBtn = document.getElementById('mark-as-read-btn');
    if (markAsReadBtn) {
        markAsReadBtn.disabled = !shouldShowDot;
    }
}
...
function renderShopItems(animate = true) {
    ...
    backgroundsContainer.innerHTML = backgrounds.map(item => {
        ...
        // CORRECTION: Autorise les fonds d'écran pour les thèmes 'default' et 'default-2'
        const isRestricted = !['default', 'theme-default-2'].includes(currentUiTheme) && item.id !== 'bg-default-theme';
        ...
        if (isRestricted) {
            itemClasses += ' locked';
            subtitle = `<span style="color: #ef4444; font-weight: 700; font-size: 0.75em;">DEFAULT THEMES ONLY</span>`;
            rightIcon = ``;
        } else if (isSelected) {
...
function updateThemeColorMeta() {
    ...
    let computedBg = getComputedStyle(document.body).getPropertyValue('--bg-color').trim();
    let themeColor = isDark ? '#121212' : (computedBg || '#ffffff');
    let navColor = isDark ? '#121212' : (computedBg || '#ffffff');
...
function applyTheme(themeName) {
    ...
    if (isBgTheme) {
        ...
        // CORRECTION: Désactive le switch de thème (clair/foncé) sauf pour les thèmes 'default' et 'default-2'
        const themeSwitch = document.getElementById('theme-switch');
        const isDefaultTheme = ['default', 'theme-default-2'].includes(themeName);
        ...
        // CORRECTION: Si on change pour un thème non-défaut et que le fond est "Icons", on le retire car il est incompatible
        if (!isDefaultTheme && safeStorage.getItem('bg-theme') === 'bg-icons') {
...
function setVolume(volume, fromUI = false) {
    ...
    if (fromUI) playAudio(sounds.select);
}
...
function setupEventListeners() {
    ...
    document.getElementById('mobile-player-progress-bar').addEventListener('click', (e) => {
        playAudio(sounds.select); // NOUVEAU: Son discret
...
// NOUVEAU: Gère le clic sur le panneau Hero (Expand ou Action)
window.handleHeroClick = function (event, panelId) {
    const panel = document.getElementById(panelId);
    // Si on clique directement sur le bouton, on laisse l'événement se propager normalement
    if (event.target.closest('.hero-button')) return;

    if (panel.classList.contains('active')) {
        // Si le panneau est déjà actif, on simule un clic sur le bouton d'action
        const btn = panel.querySelector('.hero-button');
        if (btn) {
            btn.click();
        }
    } else {
        // Sinon, on ouvre le panneau
        toggleAccordion(panelId);
    }
};

// NOUVEAU: Render Daily Bonus Widget
