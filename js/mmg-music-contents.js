...
function renderDailyBonusProgress() {
    ...
    // === NOUVEAU DESIGN WIDGET ===
    const rewardAmount = loginStreak >= 7 ? "Bonus" : `+${loginStreak || 1}`;
    ...
    const streakText = `Série ${loginStreak}/7`;

    const html = `
        <div class="bonus-content">
            <div class="bonus-info">
                ${streakText} <span style="color: #7dd3fc; margin: 0 4px;">|</span> ${rewardAmount} 
                <svg width="1em" height="1em" viewBox="0 0 100 100" xmlns="http://www.w3.org/2000/svg" style="display: inline-block; vertical-align: middle; color: #0369a1;">
                    <circle cx="50" cy="50" r="44" fill="none" stroke="currentColor" stroke-width="8" />
                    <path d="M 33.5 64 L 25 64 L 25 45 Q 25 41 27 38 Q 28.5 35 32 33.5 Q 35.5 32 39 32 Q 42 32 45 33.5 Q 48 35 50 38 Q 52 35 55 33.5 Q 58 32 61.5 32 Q 65.5 32 68.5 33.5 Q 71.5 35 73.5 38 Q 75 41 75 45 L 75 64 L 66.5 64 L 66.5 45 Q 66.5 42.5 65 41 Q 63.5 39.5 61 39.5 Q 58.5 39.5 57 41 Q 55.5 42.5 55.5 45 L 55.5 64 L 44.5 64 L 44.5 45 Q 44.5 42.5 43 41 Q 41.5 39.5 39 39.5 Q 36.5 39.5 35 41 Q 33.5 42.5 33.5 45 L 33.5 64 Z" fill="currentColor" />
                </svg>
            </div>
        </div>
        <button id="claim-bonus-btn" class="${buttonClass}" ${!bonusAvailable ? 'disabled' : ''}>${buttonIcon}</button>
    `;
...
function startHeroAccordionAutoScroll() {
    ...
    heroAccordionInterval = setInterval(() => {
        ...
        if (activePanel) {
            const nextPanel = activePanel.nextElementSibling || activePanel.parentElement.firstElementChild;
            if (nextPanel && nextPanel.classList.contains('accordion-panel')) {
...
function renderDailyBonusWidget() {
    ...
    container.innerHTML = `
        <div class="bonus-content">
            <div class="bonus-info">
                ${streakText} <span style="color: #7dd3fc; margin: 0 4px;">|</span> ${rewardAmount} ${COIN_SVG}
            </div>
        </div>
            ...
            <div class="reco-card" data-item-id="${playlist.id}">
                <div class="reco-image-wrapper">
                    <div class="reco-image-container">
                        <div class="reco-img-placeholder ${playlist.cssClass}">
                            <i class="fas fa-music"></i>
                        </div>
                    </div>
                </div>
