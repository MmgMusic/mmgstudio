...
function onPlayerStateChange(event) {
    ...
    if (event.data === YT.PlayerState.ENDED) {
        ...
        // NOUVEAU: Masquer le bouton de lecture sur la page de détails
        if (typeof updateDetailsPlayButtonState === 'function') updateDetailsPlayButtonState();
...
function loadAndPlayVideo(item, fromAutoplay = false, isSwipe = false, cueOnly = false) {
    ...
    // NOUVEAU: On montre toujours le mini-player dès qu'un morceau est chargé ou prêt (cue/load)
    const miniPlayer = document.getElementById('mobile-mini-player');
    ...
    if (activePlayer && typeof (cueOnly ? activePlayer.cueVideoById : activePlayer.loadVideoById) === 'function') {
        ...
        if (cueOnly && isSameVideo) {
            ...
            console.log("Navigation vers détails : morceau déjà actif, on ne recharge pas l'API.");
            return;
        }
...
function updateDetailsPlaylistButtonState() {
    ...
    const currentItemId = detailsSection?.dataset.currentItemId || currentPlayingItem?.id;
    ...
    if (mobileBtn) {
        mobileBtn.classList.toggle('added', isInPlaylist);
        mobileBtn.classList.toggle('fa-check', isInPlaylist);
        mobileBtn.classList.toggle('fa-plus', !isInPlaylist);
    }
}

function renderMusicTitleDetails(item, shouldPlay = false) {
    document.getElementById('music-title-details-section').dataset.currentItemId = item.id;
    const activeProfile = document.querySelector('.profile-tab.active')?.dataset.project || 'mmg-music';
    const album = siteData.contentData[activeProfile].albums[item.albumId];

    // MODIFICATION: On charge la vidéo dynamiquement dans le lecteur partagé (sans écraser le host)
    loadAndPlayVideo(item, false, false, !shouldPlay); // Si shouldPlay est true, cueOnly est false (lecture immédiate)
...
function initializeApp() {
    ...
    // NETTOYAGE: Masquer l'option "Background Play" abandonnée si elle existe dans le DOM
    const bgPlaySwitch = document.getElementById('background-play-switch');
    if (bgPlaySwitch) {
        const parent = bgPlaySwitch.closest('.setting-item');
...
function setupEventListeners() {
    ...
    // NOUVEAU: Listener pour le bouton Tout Écouter
    const albumPlayAllBtn = document.getElementById('album-play-all-btn');
    if (albumPlayAllBtn) {
        albumPlayAllBtn.addEventListener('click', (e) => {
            e.preventDefault();
            const albumId = e.currentTarget.dataset.albumId;
            if (albumId) {
                const activeProfile = document.querySelector('.profile-tab.active')?.dataset.project || 'mmg-music';
                // Récupérer tous les titres de l'album
                const allAlbumTracks = Object.values(siteData.contentData[activeProfile].titles)
                    .filter(title => title.albumId === albumId);
                
                // Filtrer les titres verrouillés si nécessaire
                const playableTracks = allAlbumTracks.filter(track => !track.isUnlockable || unlockedDreamSeasonTracks.includes(track.id));
                
                if (playableTracks.length > 0) {
                    const playlistIds = playableTracks.map(t => t.id);
                    playAudio(sounds.select);
                    // Lancer le premier titre avec la liste complète en contexte (fromAutoplay=true pour lancer direct)
                    playVideoWhenReady(playableTracks[0], playlistIds, 0, 'album', false, true);
                }
            }
        });
    }
    ...
    // NOUVEAU: Gestionnaire pour le bouton de lecture sur la page de détails
    const detailsPlayBtn = document.getElementById('details-play-btn');
    if (detailsPlayBtn) {
        detailsPlayBtn.addEventListener('click', () => {
            if (typeof playFromDetailsPage === 'function') {
                playFromDetailsPage();
                playAudio(sounds.select);
            }
        });
    ...
    document.getElementById('details-next-arrow')?.addEventListener('click', () => playNextTrack(1, true));
    document.getElementById('details-prev-arrow')?.addEventListener('click', () => playNextTrack(-1, true));
...
function handleMenuNavigation(dataLink, updateHistory = true, context = null) {
    ...
    if (dataLink === 'home-dashboard-section') {
        ...
        if (effectiveContext && effectiveContext.type === 'titles' && effectiveContext.data) {
            ...
            // NOUVEAU: Configurer et afficher le bouton Play All
            if (playAllBtn) {
                playAllBtn.style.display = 'inline-flex';
                playAllBtn.dataset.albumId = albumId;
            }
        } else {
            ...
            // Masquer le bouton si on n'est pas sur un album spécifique (par sécurité)
        ...
        return `
            <div class="carousel-item hero-carousel-item" data-item-id="${item.id}">
                ...
                <div class="hero-content">
                    ...
                    <button class="hero-button dashboard-card-button" data-youtube-id="${item.youtube_id}" data-item-id="${item.id}">${getTranslation('viewDetails')}</button>
                </div>
            </div>
        `}).join('');
