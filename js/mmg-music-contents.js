...
let newsData = {}; // NOUVEAU: Pour les actualités chargées depuis news.json
...
let sfxLock = false; // NOUVEAU: Verrou pour empêcher les sons de se chevaucher
...
const sounds = {
    ...
    typingForwards: document.getElementById('typing-forwards-sound'),
    typingBackwards: document.getElementById('typing-backwards-sound'),
    notification: document.getElementById('notification-sound'),
    tags: document.getElementById('tags-sound'),
};
...
async function loadDataAndInitialize() {
    try {
        ...
        if (!siteResponse.ok) throw new Error(`HTTP error loading data! status: ${siteResponse.status}`);
        if (!newsResponse.ok) throw new Error(`HTTP error loading news! status: ${newsResponse.status}`);
...
function onPlayerStateChange(event) {
    ...
    if (event.data === YT.PlayerState.ENDED) {
        ...
        if (currentPlayingItem && isMusicTitle(currentPlayingItem) && !seekDetectedInCurrentPlay && finalProgress >= 0.95) {
            ...
            updateCoinDisplay();
            showDialog(getTranslation('coinObtained', { total: userCoins }));
...
function initUserData() {
    ...
    const defaultAchievements = siteData.achievements || {};
    achievements = storedAchievements ? JSON.parse(storedAchievements) : JSON.parse(JSON.stringify(defaultAchievements));

    Object.keys(defaultAchievements).forEach(key => {
        if (!achievements[key]) {
            achievements[key] = JSON.parse(JSON.stringify(defaultAchievements[key]));
        }
    });
...
function updateNotificationDot() {
    ...
    // NOUVEAU: Détecter si l'état du point rouge change pour jouer le son de notification.
    // C'est le seul endroit où ce son doit être joué.
    const isCurrentlyHidden = dots.length > 0 && dots[0].classList.contains('hidden');
    if (shouldShowDot && isCurrentlyHidden) {
        playAudio(sounds.notification);
    }
...
function renderAchievements() {
    ...
    Object.entries(achievements).forEach(([id, ach]) => {
        ...
        } else if (id === 'psPlayer' || id === 'spotimonFan') { // NOUVEAU: Ajout de spotimonFan
...
function togglePlaylistItem(itemId) {
    ...
    if (itemIndex > -1) {
        ...
        showDialog(getTranslation("titleRemovedPlaylist")); // Le son de notification est déjà joué par showDialog
    } else {
        currentPlaylist.push(itemId);
        showDialog(getTranslation("titleAddedPlaylist")); // Le son de notification est déjà joué par showDialog
    }
...
function playAudio(audioElement, force = false) {
    ...
    // NOUVEAU: Verrou pour empêcher les sons de se chevaucher
    if (sfxLock) return;

    sfxLock = true;
    audioElement.currentTime = 0;
    audioElement.play().catch(error => {
        sfxLock = false; // Libère le verrou si la lecture échoue
    });

    // Libère le verrou après une courte durée.
    // Assez court pour permettre des clics rapides, assez long pour éviter les doublons dans le même événement.
    setTimeout(() => {
        sfxLock = false;
    }, 50); // 50ms lock
}
    ...
    if (currentViewContext.type) {
        ...
        switch (currentViewContext.type) {
            ...
            case 'albums':
                ...
                break;
            case 'news':
                renderNewsDetailsPage(currentViewContext.data);
                break;
            case 'videos':
                renderVideosPage();
                break;
...
function renderShopItems(animate = true) {
    ...
    themesContainer.innerHTML = visibleThemes.map(item => {
        ...
        if (item.missionId) {
            ...
            } else if (['psPlayer', 'spotimonFan'].includes(item.missionId)) {
...
// NOUVEAU: Met à jour la couleur de la barre de statut/navigation d'Android
function updateThemeColorMeta(forceIntro = false) {
    const isDark = document.body.classList.contains('dark-theme');
    const introContainer = document.getElementById('intro-container');
    const isIntroVisible = forceIntro || (introContainer && introContainer.style.display !== 'none' && introContainer.style.opacity !== '0');
    ...
    if (isIntroVisible) {
        // Pendant l'intro, tout doit être blanc pur
        themeColor = '#ffffff';
        navColor = '#ffffff';
    } else if (isDark) {
        // Mode Sombre (Default/Default 2)
        themeColor = '#121212';
        navColor = '#121212';
    } else {
        // Mode Clair et Thèmes Spéciaux
        if (document.body.classList.contains('theme-playstation')) {
            themeColor = '#002d5c'; // Top Bar PS3
            navColor = '#002d5c';   // Bottom Bar PS3 (Bleu bas)
        } else if (document.body.classList.contains('theme-music-studio')) {
            themeColor = '#1e2327'; // Top Panel Rack
            navColor = '#0f1113';   // Bottom Panel Rack
        } else if (document.body.classList.contains('theme-spotimon')) {
            themeColor = '#f4f3f3'; // Top Bar Spotimon
            navColor = '#111111';   // Bottom Bar Spotimon (Noir)
        } else if (document.body.classList.contains('theme-16bit')) {
            themeColor = '#f6fcfe'; // Frutiger Aero (Haut)
            navColor = '#ebf8ff';   // Frutiger Aero (Bas - Bleu très clair)
        } else if (document.body.classList.contains('theme-default-2')) {
            themeColor = '#34d399'; // TEST: Vert pour vérifier l'application
            navColor = '#34d399';   // TEST: Vert pour vérifier l'application
        } else {
            // Thème Default
            themeColor = '#ffffff'; // Légèrement off-white pour éviter le clash visuel
            navColor = '#f4f3f3';
        }
    }

    if (statusBarMeta) statusBarMeta.setAttribute('content', themeColor);
    if (navBarMeta) navBarMeta.setAttribute('content', navColor);
    if (colorSchemeMeta) {
...
function closeOverlay(sound) {
    // NOUVEAU: Empêche la fermeture manuelle des overlays si un guide est actif
...
window.closeAllOverlays = function () {
    ...
    activeOverlay = null;
    // CORRECTION: Ne plus retirer les classes de guide ici pour éviter de casser le guide en cours.
    // C'est GuideManager.stop() qui s'en charge.
};
...
function showDialog(message, playSound = false) {
    const dialog = document.getElementById('custom-dialog');
    dialog.querySelector('p').textContent = message;
    if (playSound) playAudio(sounds.notification); // NOTE: Le son de notification est maintenant géré par updateNotificationDot()
...
function applyInitialTheme() {
    // MODIFICATION: Le mode clair est maintenant forcé par défaut pour éviter la détection système.
    let savedTheme = safeStorage.getItem('mmg-theme') || 'light';
...
function renderSidebarNav(activeProfile, currentSectionId = null) {
    ...
    const profileData = Object.values(siteData.projectData[activeProfile] || {}).filter(item => item && item.link);
...
function switchProfile(targetProfile) {
    ...
    if (commonSections.includes(currentSectionId)) {
        ...
        if (siteData.projectData && siteData.projectData[previousProfile]) {
            const prevItem = Object.values(siteData.projectData[previousProfile]).find(p => p && p.link === currentSectionId);
            if (prevItem) {
                const equivalentSection = Object.values(siteData.projectData[targetProfile]).find(item => {
                    return item && (item.langKey === prevItem.langKey || item.link === prevItem.link);
                });
...
function setupEventListeners() {
    ...
    if (detailsOptionsBtn) {
        detailsOptionsBtn.addEventListener('click', (e) => {
            ...
            playAudio(sounds.select); // NOUVEAU: son
            const currentItemId = document.getElementById('music-title-details-section').dataset.currentItemId;
    ...
    // NOUVEAU: Écouteurs pour les sons de frappe
    const inputsWithTypingSound = document.querySelectorAll('#search-input, #mobile-search-input, #cheat-code-input');
    inputsWithTypingSound.forEach(input => {
        input.addEventListener('keydown', (e) => {
            // On ne joue pas de son pour les touches de contrôle/navigation
            if (e.ctrlKey || e.altKey || e.metaKey || (e.key.length > 1 && e.key !== 'Backspace')) {
                return;
            }

            if (e.key === 'Backspace') {
                playAudio(sounds.typingBackwards);
            } else {
                // Pour les autres touches (lettres, chiffres, symboles), on joue le son "forwards"
                playAudio(sounds.typingForwards);
            }
        });
    });

    // NOUVEAU: Gestion intelligente des sons pour les tags (Sélection vs Désélection)
    document.getElementById('tags-filter-list').addEventListener('change', (e) => {
        if (e.target.tagName === 'INPUT') {
            if (e.target.checked) {
                playAudio(sounds.tags); // Son normal quand on coche
            } else {
                playAudio(sounds.typingBackwards); // Son "effacer" quand on décoche
            }
        }
        updateVisibleCards();
    });
    ...
    document.getElementById('mini-player-close-btn').addEventListener('click', (e) => {
        e.stopPropagation();
        playAudio(sounds.back); // NOUVEAU: son de fermeture
        const wasVideo = currentPlayingItem && !isMusicTitle(currentPlayingItem);
        if (activePlayer && typeof activePlayer.stopVideo === 'function') activePlayer.stopVideo();
        currentPlayingItem = null;
        resetMiniPlayerUI();
        if (wasVideo) { history.back(); }
    });
    ...
    document.getElementById('share-btn').addEventListener('click', () => { playAudio(sounds.select); shareFunction(); });
    document.getElementById('overlay-share-btn').addEventListener('click', () => { playAudio(sounds.select); shareFunction(); });


    document.getElementById('player-like-btn').addEventListener('click', (e) => {
        if (currentPlayingItem) {
            playAudio(sounds.select); // NOUVEAU: son de like
            toggleLike(currentPlayingItem.id);
        }
    ...
    document.getElementById('wifi-btn-settings').addEventListener('click', (e) => { e.preventDefault(); playAudio(sounds.select); openOverlay(document.getElementById('wifi-overlay'), null); });
    ...
    allOverlays.forEach(overlay => {
        overlay.addEventListener('click', (e) => {
            // NOUVEAU: Bloque la fermeture si guide actif
    ...
    document.getElementById('cheat-code-btn').addEventListener('click', () => {
        ...
        if (code === 'gameshark') { // If cheat code is gameshark
            ...
            updateCoinDisplay();
            input.value = '';
            showDialog(getTranslation('coinsObtained', { count: 10, total: userCoins }));
            playAudio(sounds.coin);
            ...
            showDialog(getTranslation('incorrectCode'), false); // Pas de son de notif, car on joue 'blocked' juste après
            playAudio(sounds.blocked);
        }
    ...
    let lastWasPC = window.innerWidth > 952;
    window.addEventListener('resize', () => {
        const isMobileNow = window.innerWidth <= 952;
        const isPCNow = !isMobileNow;
        ...
        // BUG FIX: Si on resize de PC vers Mobile ET qu'on est sur le grand lecteur vidéo,
        // on redirige vers la page d'accueil pour éviter l'affichage incorrect
        if (isMobileNow && lastWasPC) {
            const largePlayerSection = document.getElementById('large-player-section');
            if (largePlayerSection && !largePlayerSection.classList.contains('hidden')) {
                // On est en mode video PC et on passe en mobile → retour accueil
                handleMenuNavigation('home-dashboard-section', false);
            }
        }

        // BUG FIX: Si on resize de Mobile vers PC, réinitialiser les transforms des overlays
        // pour éviter qu'ils restent collés en bas de l'écran (translateY(0) du mode mobile)
        if (isPCNow && !lastWasPC) {
...
function updateMobileHeader(sectionId) {
    ...
    const staticSectionTitles = {
        ...
        'upcoming-release-section': getTranslation('mobileUpcoming'),
        'news-details-section': getTranslation('news'),
    };
