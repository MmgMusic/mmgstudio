...
function setupEventListeners() {
    ...
    if (mobileProfileSwitch) {
        mobileProfileSwitch.addEventListener('click', (e) => {
            ...
            if (desktopTab) {
                // CORRECTION: Définition de topBar avant utilisation
                const topBar = document.querySelector('.top-bar');
                // NOUVEAU: On s'assure que la recherche est fermée lors d'un switch de profil pour éviter les bugs visuels
                if (topBar && topBar.classList.contains('searching') && currentViewContext.type !== 'search') {
                    toggleSearch(); // Force close (toggleSearch handles checking state now)
                }
    ...
    container.innerHTML = `
        <div class="track-details-container" style="padding: 20px; max-width: 800px; margin: 0 auto;">
            <div class="track-details-header" style="text-align: center; margin-bottom: 24px;">
                ...
                <p style="color: var(--primary-color); font-weight: 600; font-size: 0.9rem; margin-bottom: 8px;"><i class="far fa-calendar-alt"></i> ${upcomingData.date}</p>
            </div>
            <div class="track-details-info" style="line-height: 1.6; font-size: 1rem; color: var(--text-main opacity: 0.9); text-align: justify;">
...
function toggleSearch() {
    ...
    topBar.classList.toggle('searching');
    ...
    if (topBar.classList.contains('searching')) {
        ...
        // En fermant, on vide l'input
        const input = document.getElementById('mobile-search-input');
        if (input) input.value = '';
    }
}

// Fermer la recherche au clic extérieur (Global Listener comme dans FinalUI)
document.addEventListener('click', (e) => {
    const topBar = document.querySelector('.top-bar');
    // On vérifie que le clic n'est PAS dans la top bar (header)
    // Mais attention : le bouton qui ouvre la recherche EST dans la top bar.
    // Le code FinalUI dit : !header.contains(e.target).
    // Cela signifie que si on clique n'importe où DANS le header (boutons, titres), ça ne ferme pas.
    // Ça ferme seulement si on clique dans le contenu principal.
...
// Setup listeners for the new mobile search
document.addEventListener('DOMContentLoaded', () => {
    const mobileSearchBtn = document.getElementById('mobile-search-btn');
