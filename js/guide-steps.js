...
window.GUIDE_STEPS = {
    ...
    navigation: [
        {
            ...
            content: {
                fr: "Ce guide présente les éléments de l'interface MMGEAR. Passez instantanément de MMG MUSIC à MMG BEATS en cliquant sur le logo.",
                en: "This guide presents the MMGEAR interface elements. Switch instantly from MMG MUSIC to MMG BEATS by clicking the logo."
            },
            position: 'right',
            onEnter: () => {
                ...
                // Placeholder pochette activé dès le début
                setTimeout(() => {
                    const cover = document.getElementById('player-album-cover');
                    if (cover) {
                        cover.src = 'assets/mmg-music-avatar.webp';
                        // On s'assure que le mini-lecteur mobile affiche aussi quelque chose de cohérent
                        const miniArt = document.getElementById('mini-player-album-art');
                        if (miniArt) miniArt.src = 'assets/mmg-music-avatar.webp';
                        const mobileArt = document.getElementById('mobile-player-album-art');
                        if (mobileArt) mobileArt.src = 'assets/mmg-music-avatar.webp';
                    }
                }, 200);
            }
        },
        ...
        },
        {
            ...
            content: {
                fr: "Allumez ou éteignez l'autoplay (lecture auto au clic). Désactivé = naviguer sans lancer le contenu pour lire les pages de détails. Gérez aussi le volume, la file d'attente et le partage ici.",
                en: "Turn autoplay on or off. Off = browse without starting playback to read detail pages. Also manage volume, queue, and sharing here."
            },
            position: 'top'
        }
    ],
    ...
    pages: [
        ...
        },
        {
            ...
            targets: ['#music-title-details-section'],
            ...
            position: 'top',
            delay: 1500, // Augmenté pour laisser le temps à la navigation complexe
            onEnter: () => {
                // 1. Navigation vers les albums
                if (window.handleMenuNavigation) window.handleMenuNavigation('albums-section');

                setTimeout(() => {
                    // 2. Chercher l'album "The Red Drum Vol. 1" par son ID fiable
                    const targetAlbum = document.querySelector('.card[data-item-id="album2"]');

                    if (targetAlbum) {
                        // Feedback visuel du clic
                        targetAlbum.classList.add('guide-click-feedback');

                        setTimeout(() => {
                            targetAlbum.classList.remove('guide-click-feedback');

                            // CRITICAL: Cliquer sur le lien interne pour le listener délégué
                            const albumLink = targetAlbum.querySelector('.card-link-wrapper');
                            if (albumLink) albumLink.click(); else targetAlbum.click();

                            // Attendre le rendu des titres
                            setTimeout(() => {
                                // 3. Chercher le titre "La Rencontre" par son ID fiable
                                ...
                                if (targetTrack) {
                                    ...
                                    setTimeout(() => {
                                        ...
                                        // Désactiver temporairement l'autoplay via window pour ne pas lancer le son
                                        const wasAutoplay = (typeof window.isAutoplayActive !== 'undefined') ? window.isAutoplayActive : true;
                                        if (typeof window.isAutoplayActive !== 'undefined') window.isAutoplayActive = false;
                                        ...
                                        setTimeout(() => {
                                            ...
                                            // Restaurer l'autoplay après l'affichage
                                            setTimeout(() => {
                                                if (typeof window.isAutoplayActive !== 'undefined') window.isAutoplayActive = wasAutoplay;
                                            }, 500);
                                        }, 400);
                                    }, 800);
                                }
                            }, 1200); // Temps de chargement des titres
                        }, 800);
                    }
                }, 1000); // Temps de chargement des albums
            }
        },
        {
            id: 'page_videos',
        ...
        },
        {
            ...
            content: {
                fr: "Vos likes, les playlists MMGEAR et vos propres listes enregistrées.",
                en: "Your likes, MMGEAR playlists, and your own saved lists."
            },
            position: 'top',
            delay: 800,
            onEnter: () => {
    ...
    navigation_mobile: [
        {
            ...
            content: { fr: "L'essentiel de MMGEAR tient dans votre poche.", en: "MMGEAR essentials in your pocket." },
            position: 'bottom'
        }
    ],
