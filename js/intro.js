/**
 * INTRO FINALE INTERACTIVE
 * Gère l'animation d'entrée et le passage au contenu principal.
 * Version sans Tailwind — utilise les classes CSS natives de intro-styles-natifs.css
 */
document.addEventListener('DOMContentLoaded', () => {
    const el = {
        music:       document.getElementById('music-text'),
        beats:       document.getElementById('beats-text'),
        studio:      document.getElementById('studio-wrapper'),
        logo:        document.getElementById('logo-box'),
        parts:       document.getElementById('particles-container'),
        container:   document.getElementById('intro-container'),
        scaler:      document.getElementById('scene-scaler'),
        mainContent: document.getElementById('main-content-wrapper')
    };

    if (!el.container) return;

    // Correspondance icône → classe couleur CSS native (plus de classes Tailwind)
    const icons = [
        { svg: '<path d="M9 18V5l12-2v13"/><circle cx="6" cy="18" r="3"/><circle cx="18" cy="16" r="3"/>', color: 'intro-particle-blue',    angle: 0   },
        { svg: '<path d="M23 7l-7 5 7 5V7z"/><rect x="1" y="5" width="15" height="14" rx="2" ry="2"/>',   color: 'intro-particle-purple',  angle: 45  },
        { svg: '<rect x="2" y="2" width="20" height="20" rx="2" ry="2"/><path d="M7 2v20M17 2v20M2 12h20"/>',color: 'intro-particle-red',   angle: 90  },
        { svg: '<path d="M12 6v14M8 8v12M4 4v16M16 6l4 14"/>',                                             color: 'intro-particle-emerald', angle: 135 },
        { svg: '<circle cx="12" cy="13" r="4"/><path d="M23 19a2 2 0 0 1-2 2H3a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h4l2-3h6l2 3h4a2 2 0 0 1 2 2z"/>', color: 'intro-particle-pink', angle: 180 },
        { svg: '<path d="M12 2.69l5.66 5.66a8 8 0 1 1-11.31 0z"/>',                                        color: 'intro-particle-orange',  angle: 225 },
        { svg: '<polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2"/>',                               color: 'intro-particle-yellow',  angle: 270 },
        { svg: '<path d="M12 3l1.912 5.813a2 2 0 0 0 1.275 1.275L21 12l-5.813 1.912a2 2 0 0 0-1.275 1.275L12 21l-1.912-5.813a2 2 0 0 0-1.275-1.275L3 12l5.813-1.912a2 2 0 0 0 1.275-1.275L12 3z"/>', color: 'intro-particle-cyan', angle: 315 }
    ];

    let canEnter = false;
    let isExiting = false;

    function handleResize() {
        const baseWidth = 1200;
        const scale = Math.min(0.85, (window.innerWidth - 40) / baseWidth);
        if (el.scaler) el.scaler.style.transform = `scale(${scale})`;
    }

    window.addEventListener('resize', handleResize);
    handleResize();

    function triggerExplosion() {
        const fragment = document.createDocumentFragment();
        icons.forEach((icon, i) => {
            const wrap = document.createElement('div');
            // particle-wrapper défini dans le <style> de l'index, couleur via classe CSS native
            wrap.className = `particle-wrapper ${icon.color}`;
            wrap.style.setProperty('--angle', (icon.angle - 90) + 'deg');
            wrap.style.animationDelay = `${i * 0.04}s`;

            const inner = document.createElement('div');
            inner.innerHTML = `<svg xmlns="http://www.w3.org/2000/svg" width="46" height="46" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round">${icon.svg}</svg>`;
            wrap.appendChild(inner);
            fragment.appendChild(wrap);

            setTimeout(() => inner.classList.add('is-floating'), 1500);
        });
        el.parts.appendChild(fragment);
        setTimeout(() => {
            canEnter = true;
            startApp();
        }, 2000);
    }

    function startApp() {
        if (!canEnter || isExiting) return;
        isExiting = true;

        // Sortie : fade out du conteneur
        el.container.style.opacity = '0';

        // Studio : transition de sortie via style inline (état ponctuel, pas de classe dédiée)
        el.studio.style.transition = 'all 0.8s ease-in-out';
        el.studio.style.transform = 'translateY(15px) scale(1.1)';
        el.studio.style.filter = 'blur(12px)';

        // Particules : propulsion de sortie
        document.querySelectorAll('.particle-wrapper').forEach(w => w.classList.add('exit-propulsion'));

        setTimeout(() => {
            el.container.style.display = 'none';
            if (el.mainContent) el.mainContent.style.opacity = '1';
            window.removeEventListener('resize', handleResize);
        }, 800);
    }

    // Séquence d'animation
    (async () => {
        if (typeof updateThemeColorMeta === 'function') {
            updateThemeColorMeta(true);
        }
        await document.fonts.ready;
        el.container.style.opacity = '1';

        // 1. Logo tombe
        setTimeout(() => {
            el.logo.style.opacity = '1';
            el.logo.style.animation = 'physics-bounce 1.6s forwards var(--anim-bezier)';
        }, 400);

        // 2. Textes glissent depuis les côtés
        setTimeout(() => {
            el.music.style.opacity = '1';
            el.beats.style.opacity = '1';
            el.music.style.filter = 'blur(0)';
            el.beats.style.filter = 'blur(0)';
            el.music.style.transform = 'translateX(-260px)';
            el.beats.style.transform = 'translateX(260px)';
        }, 2000);

        // 3. Collision : textes convergent et disparaissent, studio apparaît
        setTimeout(() => {
            const trans = 'transform 0.5s var(--collision-bezier), opacity 0.3s ease-in, filter 0.3s';
            [el.music, el.beats].forEach(e => {
                e.style.transition = trans;
                e.style.transform = 'translateX(0) scale(1.4)';
                e.style.opacity = '0';
                e.style.filter = 'blur(15px)';
            });

            el.logo.style.animation = 'collision-flash 0.6s ease-out forwards';

            setTimeout(() => {
                // Révèle le studio via classe CSS native (remplace les 3 remove/add Tailwind)
                el.studio.classList.add('intro-visible');
                triggerExplosion();
            }, 80);
        }, 4400);
    })();
});
