...
class GuideManager {
    ...
    stop() {
        ...
        // CORRECTION: Débloque le site à la fin du guide
        document.body.classList.remove('tutorial-active-body');
        document.documentElement.classList.remove('tutorial-active-body');

        // Nettoyage des feedbacks visuels
        document.querySelectorAll('.guide-click-feedback').forEach(el => el.classList.remove('guide-click-feedback'));

        // NOUVEAU: S'assurer que les événements ne restent pas bloqués (YouTube)
        ...
        // On ferme tout overlay de sélection qui pourrait rester
    ...
    updateMarkerFromRect(rect) {
        ...
        const offset = 0; // Marge réduite pour un encadrement plus précis
