...
class GuideManager {
    ...
    init() {
        ...
        if (!this.tooltip || !document.body.contains(this.tooltip)) {
            ...
            // NOUVEAU: Cliquer sur l'overlay (en dehors du tooltip) ferme le guide
    ...
    showSplash(guideKey) {
        ...
        } else if (guideKey.startsWith('tips')) {
            descKey = 'guide_desc_tips';
        }
    ...
    stop() {
        this.clearTimeouts(); // NOUVEAU: Arrete tout ce qui est en cours

        // Cacher tous les éléments du guide
        ...
        // DÉBLOCAGE COMPLET ET AGGRESSIF DU SITE
        const body = document.body;
        const html = document.documentElement;

        const tutorialClasses = ['tutorial-active-body', 'guide-active'];
        tutorialClasses.forEach(cls => {
            body.classList.remove(cls);
            html.classList.remove(cls);
        });

        // Reset des styles inline qui pourraient bloquer
        body.style.overflow = '';
        body.style.pointerEvents = 'auto'; // Force 'auto' au lieu de vide pour écraser tout reste
        ...
        // NOUVEAU: S'assurer que les conteneurs principaux sont cliquables
        const resetElements = [
            document.getElementById('main-content-wrapper'),
            document.getElementById('main-area'),
            document.getElementById('mobile-mini-player'),
        ...
        resetElements.forEach(el => {
            if (el) {
                ...
                el.style.opacity = '1';
                el.classList.remove('guide-blur', 'guide-dimmed');
            }
        });

        // Fermer les overlays de secours
        if (typeof window.closeAllOverlays === 'function') {
            window.closeAllOverlays();
        } else {
            document.querySelectorAll('.overlay, [id$="-overlay"], #guide-selection-overlay').forEach(ov => {
                ov.classList.add('hidden');
            });
        }

        // NOUVEAU: Fermer spécifiquement le lecteur mobile s'il a été ouvert par un pas du guide
        const mobilePlayer = document.getElementById('mobile-full-player');
        if (mobilePlayer) {
            mobilePlayer.classList.remove('active');
        }

        // Nettoyage des classes résiduelles sur les éléments du site
        ...
        // Force le navigateur à rafraîchir le rendu pour appliquer les changements de pointer-events
    ...
    updateMarkerFromRect(rect) {
        ...
        const offset = 8; // Marge augmentée pour aérer la mise en valeur
